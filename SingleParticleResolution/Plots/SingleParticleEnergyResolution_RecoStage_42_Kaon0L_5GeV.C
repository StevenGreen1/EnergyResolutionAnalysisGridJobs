{
//=========Macro generated from canvas: Name/Title
//=========  (Fri Oct 23 09:13:28 2015) by ROOT version5.34/30
   TCanvas *Name = new TCanvas("Name", "Title",0,0,600,500);
   gStyle->SetOptStat(0);
   Name->SetHighLightColor(2);
   Name->Range(-1.25,-72.84376,11.25,655.5938);
   Name->SetFillColor(0);
   Name->SetBorderMode(0);
   Name->SetBorderSize(2);
   Name->SetTickx(1);
   Name->SetTicky(1);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   
   TH1F *Name = new TH1F("Name","Title",50,0,10);
   Name->SetBinContent(2,1);
   Name->SetBinContent(3,4);
   Name->SetBinContent(4,6);
   Name->SetBinContent(5,6);
   Name->SetBinContent(6,5);
   Name->SetBinContent(7,6);
   Name->SetBinContent(8,9);
   Name->SetBinContent(9,20);
   Name->SetBinContent(10,18);
   Name->SetBinContent(11,36);
   Name->SetBinContent(12,69);
   Name->SetBinContent(13,82);
   Name->SetBinContent(14,136);
   Name->SetBinContent(15,185);
   Name->SetBinContent(16,266);
   Name->SetBinContent(17,325);
   Name->SetBinContent(18,370);
   Name->SetBinContent(19,437);
   Name->SetBinContent(20,541);
   Name->SetBinContent(21,493);
   Name->SetBinContent(22,555);
   Name->SetBinContent(23,521);
   Name->SetBinContent(24,441);
   Name->SetBinContent(25,451);
   Name->SetBinContent(26,396);
   Name->SetBinContent(27,342);
   Name->SetBinContent(28,273);
   Name->SetBinContent(29,245);
   Name->SetBinContent(30,200);
   Name->SetBinContent(31,152);
   Name->SetBinContent(32,122);
   Name->SetBinContent(33,102);
   Name->SetBinContent(34,84);
   Name->SetBinContent(35,57);
   Name->SetBinContent(36,41);
   Name->SetBinContent(37,43);
   Name->SetBinContent(38,23);
   Name->SetBinContent(39,22);
   Name->SetBinContent(40,20);
   Name->SetBinContent(41,19);
   Name->SetBinContent(42,10);
   Name->SetBinContent(43,12);
   Name->SetBinContent(44,6);
   Name->SetBinContent(45,9);
   Name->SetBinContent(46,7);
   Name->SetBinContent(47,1);
   Name->SetBinContent(48,8);
   Name->SetBinContent(49,1);
   Name->SetBinContent(50,1);
   Name->SetBinContent(51,12);
   Name->SetEntries(7191);
   Name->SetStats(0);
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,10);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(223.4288);
   Gaussian->SetNDF(46);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,505.6938);
   Gaussian->SetParError(0,7.909273);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,4.459899);
   Gaussian->SetParError(1,0.01411257);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,1.097508);
   Gaussian->SetParError(2,0.0110267);
   Gaussian->SetParLimits(2,0,12.03996);
   Name->GetListOfFunctions()->Add(Gaussian);
   Name->SetLineWidth(2);
   Name->SetMarkerStyle(20);
   Name->GetXaxis()->SetTitle("PFO Energy [GeV]");
   Name->GetXaxis()->SetNdivisions(505);
   Name->GetXaxis()->SetLabelFont(132);
   Name->GetXaxis()->SetLabelSize(0.03);
   Name->GetXaxis()->SetTitleSize(0.036);
   Name->GetXaxis()->SetTitleFont(132);
   Name->GetYaxis()->SetTitle("Entries");
   Name->GetYaxis()->SetLabelFont(132);
   Name->GetYaxis()->SetLabelSize(0.03);
   Name->GetYaxis()->SetTitleSize(0.036);
   Name->GetYaxis()->SetTitleFont(132);
   Name->GetZaxis()->SetLabelFont(132);
   Name->GetZaxis()->SetLabelSize(0.03);
   Name->GetZaxis()->SetTitleSize(0.036);
   Name->GetZaxis()->SetTitleFont(132);
   Name->Draw("");
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,50);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(223.4288);
   Gaussian->SetNDF(46);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,505.6938);
   Gaussian->SetParError(0,7.909273);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,4.459899);
   Gaussian->SetParError(1,0.01411257);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,1.097508);
   Gaussian->SetParError(2,0.0110267);
   Gaussian->SetParLimits(2,0,12.03996);
   Gaussian->Draw("same");
   
   TPaveText *pt = new TPaveText(0.01,0.945,0.1004698,0.995,"blNDC");
   pt->SetName("title");
   pt->SetBorderSize(1);
   pt->SetFillColor(0);
   pt->SetLineWidth(2);
   pt->SetTextFont(132);
   TText *text = pt->AddText("Title");
   pt->Draw();
   Name->Modified();
   Name->cd();
   Name->SetSelected(Name);
}
