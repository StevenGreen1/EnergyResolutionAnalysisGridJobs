{
//=========Macro generated from canvas: Name/Title
//=========  (Tue Oct 20 18:44:31 2015) by ROOT version5.34/30
   TCanvas *Name = new TCanvas("Name", "Title",0,0,600,500);
   gStyle->SetOptStat(0);
   Name->SetHighLightColor(2);
   Name->Range(-1.25,-66.54375,11.25,598.8938);
   Name->SetFillColor(0);
   Name->SetBorderMode(0);
   Name->SetBorderSize(2);
   Name->SetTickx(1);
   Name->SetTicky(1);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   
   TH1F *Name = new TH1F("Name","Title",50,0,10);
   Name->SetBinContent(2,2);
   Name->SetBinContent(3,12);
   Name->SetBinContent(4,8);
   Name->SetBinContent(5,13);
   Name->SetBinContent(6,13);
   Name->SetBinContent(7,14);
   Name->SetBinContent(8,26);
   Name->SetBinContent(9,32);
   Name->SetBinContent(10,56);
   Name->SetBinContent(11,92);
   Name->SetBinContent(12,127);
   Name->SetBinContent(13,182);
   Name->SetBinContent(14,244);
   Name->SetBinContent(15,305);
   Name->SetBinContent(16,344);
   Name->SetBinContent(17,390);
   Name->SetBinContent(18,456);
   Name->SetBinContent(19,496);
   Name->SetBinContent(20,479);
   Name->SetBinContent(21,507);
   Name->SetBinContent(22,455);
   Name->SetBinContent(23,393);
   Name->SetBinContent(24,417);
   Name->SetBinContent(25,396);
   Name->SetBinContent(26,327);
   Name->SetBinContent(27,295);
   Name->SetBinContent(28,242);
   Name->SetBinContent(29,211);
   Name->SetBinContent(30,176);
   Name->SetBinContent(31,137);
   Name->SetBinContent(32,117);
   Name->SetBinContent(33,95);
   Name->SetBinContent(34,70);
   Name->SetBinContent(35,66);
   Name->SetBinContent(36,51);
   Name->SetBinContent(37,34);
   Name->SetBinContent(38,31);
   Name->SetBinContent(39,15);
   Name->SetBinContent(40,17);
   Name->SetBinContent(41,11);
   Name->SetBinContent(42,6);
   Name->SetBinContent(43,3);
   Name->SetBinContent(44,7);
   Name->SetBinContent(45,7);
   Name->SetBinContent(46,3);
   Name->SetBinContent(47,2);
   Name->SetBinContent(49,2);
   Name->SetBinContent(50,2);
   Name->SetBinContent(51,2);
   Name->SetEntries(7388);
   Name->SetStats(0);
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,10);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(193.031);
   Gaussian->SetNDF(45);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,472.2154);
   Gaussian->SetParError(0,7.205809);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,4.239814);
   Gaussian->SetParError(1,0.0154192);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,1.215909);
   Gaussian->SetParError(2,0.01183022);
   Gaussian->SetParLimits(2,0,12.78878);
   Name->GetListOfFunctions()->Add(Gaussian);
   Name->SetLineWidth(2);
   Name->SetMarkerStyle(20);
   Name->GetXaxis()->SetTitle("PFO Energy [GeV]");
   Name->GetXaxis()->SetNdivisions(505);
   Name->GetXaxis()->SetLabelFont(132);
   Name->GetXaxis()->SetLabelSize(0.03);
   Name->GetXaxis()->SetTitleSize(0.036);
   Name->GetXaxis()->SetTitleFont(132);
   Name->GetYaxis()->SetTitle("Entries");
   Name->GetYaxis()->SetLabelFont(132);
   Name->GetYaxis()->SetLabelSize(0.03);
   Name->GetYaxis()->SetTitleSize(0.036);
   Name->GetYaxis()->SetTitleFont(132);
   Name->GetZaxis()->SetLabelFont(132);
   Name->GetZaxis()->SetLabelSize(0.03);
   Name->GetZaxis()->SetTitleSize(0.036);
   Name->GetZaxis()->SetTitleFont(132);
   Name->Draw("");
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,50);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(193.031);
   Gaussian->SetNDF(45);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,472.2154);
   Gaussian->SetParError(0,7.205809);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,4.239814);
   Gaussian->SetParError(1,0.0154192);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,1.215909);
   Gaussian->SetParError(2,0.01183022);
   Gaussian->SetParLimits(2,0,12.78878);
   Gaussian->Draw("same");
   
   TPaveText *pt = new TPaveText(0.01,0.945,0.1004698,0.995,"blNDC");
   pt->SetName("title");
   pt->SetBorderSize(1);
   pt->SetFillColor(0);
   pt->SetLineWidth(2);
   pt->SetTextFont(132);
   TText *text = pt->AddText("Title");
   pt->Draw();
   Name->Modified();
   Name->cd();
   Name->SetSelected(Name);
}
