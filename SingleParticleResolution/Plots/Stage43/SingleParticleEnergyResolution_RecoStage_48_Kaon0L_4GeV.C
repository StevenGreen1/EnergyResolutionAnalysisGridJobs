{
//=========Macro generated from canvas: Name/Title
//=========  (Tue Oct 20 18:44:53 2015) by ROOT version5.34/30
   TCanvas *Name = new TCanvas("Name", "Title",0,0,600,500);
   gStyle->SetOptStat(0);
   Name->SetHighLightColor(2);
   Name->Range(-1,-66.54375,9,598.8938);
   Name->SetFillColor(0);
   Name->SetBorderMode(0);
   Name->SetBorderSize(2);
   Name->SetTickx(1);
   Name->SetTicky(1);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   
   TH1F *Name = new TH1F("Name","Title",50,0,8);
   Name->SetBinContent(2,2);
   Name->SetBinContent(3,9);
   Name->SetBinContent(4,20);
   Name->SetBinContent(5,19);
   Name->SetBinContent(6,21);
   Name->SetBinContent(7,38);
   Name->SetBinContent(8,52);
   Name->SetBinContent(9,78);
   Name->SetBinContent(10,98);
   Name->SetBinContent(11,127);
   Name->SetBinContent(12,197);
   Name->SetBinContent(13,249);
   Name->SetBinContent(14,292);
   Name->SetBinContent(15,355);
   Name->SetBinContent(16,384);
   Name->SetBinContent(17,437);
   Name->SetBinContent(18,481);
   Name->SetBinContent(19,507);
   Name->SetBinContent(20,459);
   Name->SetBinContent(21,488);
   Name->SetBinContent(22,441);
   Name->SetBinContent(23,449);
   Name->SetBinContent(24,378);
   Name->SetBinContent(25,318);
   Name->SetBinContent(26,280);
   Name->SetBinContent(27,280);
   Name->SetBinContent(28,225);
   Name->SetBinContent(29,212);
   Name->SetBinContent(30,140);
   Name->SetBinContent(31,135);
   Name->SetBinContent(32,93);
   Name->SetBinContent(33,86);
   Name->SetBinContent(34,65);
   Name->SetBinContent(35,54);
   Name->SetBinContent(36,48);
   Name->SetBinContent(37,31);
   Name->SetBinContent(38,29);
   Name->SetBinContent(39,17);
   Name->SetBinContent(40,17);
   Name->SetBinContent(41,11);
   Name->SetBinContent(42,14);
   Name->SetBinContent(43,7);
   Name->SetBinContent(44,6);
   Name->SetBinContent(45,10);
   Name->SetBinContent(46,5);
   Name->SetBinContent(47,3);
   Name->SetBinContent(48,1);
   Name->SetBinContent(49,5);
   Name->SetBinContent(50,4);
   Name->SetBinContent(51,12);
   Name->SetEntries(7689);
   Name->SetStats(0);
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,8);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(187.6718);
   Gaussian->SetNDF(46);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,469.8716);
   Gaussian->SetParError(0,7.009332);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,3.241659);
   Gaussian->SetParError(1,0.01254128);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,1.018659);
   Gaussian->SetParError(2,0.009711663);
   Gaussian->SetParLimits(2,0,10.81401);
   Name->GetListOfFunctions()->Add(Gaussian);
   Name->SetLineWidth(2);
   Name->SetMarkerStyle(20);
   Name->GetXaxis()->SetTitle("PFO Energy [GeV]");
   Name->GetXaxis()->SetNdivisions(505);
   Name->GetXaxis()->SetLabelFont(132);
   Name->GetXaxis()->SetLabelSize(0.03);
   Name->GetXaxis()->SetTitleSize(0.036);
   Name->GetXaxis()->SetTitleFont(132);
   Name->GetYaxis()->SetTitle("Entries");
   Name->GetYaxis()->SetLabelFont(132);
   Name->GetYaxis()->SetLabelSize(0.03);
   Name->GetYaxis()->SetTitleSize(0.036);
   Name->GetYaxis()->SetTitleFont(132);
   Name->GetZaxis()->SetLabelFont(132);
   Name->GetZaxis()->SetLabelSize(0.03);
   Name->GetZaxis()->SetTitleSize(0.036);
   Name->GetZaxis()->SetTitleFont(132);
   Name->Draw("");
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,50);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(187.6718);
   Gaussian->SetNDF(46);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,469.8716);
   Gaussian->SetParError(0,7.009332);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,3.241659);
   Gaussian->SetParError(1,0.01254128);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,1.018659);
   Gaussian->SetParError(2,0.009711663);
   Gaussian->SetParLimits(2,0,10.81401);
   Gaussian->Draw("same");
   
   TPaveText *pt = new TPaveText(0.01,0.945,0.1004698,0.995,"blNDC");
   pt->SetName("title");
   pt->SetBorderSize(1);
   pt->SetFillColor(0);
   pt->SetLineWidth(2);
   pt->SetTextFont(132);
   TText *text = pt->AddText("Title");
   pt->Draw();
   Name->Modified();
   Name->cd();
   Name->SetSelected(Name);
}
