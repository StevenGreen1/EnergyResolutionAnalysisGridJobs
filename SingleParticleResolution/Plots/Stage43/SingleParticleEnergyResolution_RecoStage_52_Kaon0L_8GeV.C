{
//=========Macro generated from canvas: Name/Title
//=========  (Tue Oct 20 18:45:26 2015) by ROOT version5.34/30
   TCanvas *Name = new TCanvas("Name", "Title",0,0,600,500);
   gStyle->SetOptStat(0);
   Name->SetHighLightColor(2);
   Name->Range(-2,-75.73126,18,681.5813);
   Name->SetFillColor(0);
   Name->SetBorderMode(0);
   Name->SetBorderSize(2);
   Name->SetTickx(1);
   Name->SetTicky(1);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   
   TH1F *Name = new TH1F("Name","Title",50,0,16);
   Name->SetBinContent(6,1);
   Name->SetBinContent(7,4);
   Name->SetBinContent(8,5);
   Name->SetBinContent(9,3);
   Name->SetBinContent(10,5);
   Name->SetBinContent(11,15);
   Name->SetBinContent(12,22);
   Name->SetBinContent(13,35);
   Name->SetBinContent(14,62);
   Name->SetBinContent(15,113);
   Name->SetBinContent(16,157);
   Name->SetBinContent(17,237);
   Name->SetBinContent(18,353);
   Name->SetBinContent(19,400);
   Name->SetBinContent(20,479);
   Name->SetBinContent(21,516);
   Name->SetBinContent(22,560);
   Name->SetBinContent(23,577);
   Name->SetBinContent(24,537);
   Name->SetBinContent(25,516);
   Name->SetBinContent(26,482);
   Name->SetBinContent(27,388);
   Name->SetBinContent(28,346);
   Name->SetBinContent(29,300);
   Name->SetBinContent(30,215);
   Name->SetBinContent(31,177);
   Name->SetBinContent(32,147);
   Name->SetBinContent(33,95);
   Name->SetBinContent(34,83);
   Name->SetBinContent(35,57);
   Name->SetBinContent(36,34);
   Name->SetBinContent(37,29);
   Name->SetBinContent(38,17);
   Name->SetBinContent(39,5);
   Name->SetBinContent(40,11);
   Name->SetBinContent(41,4);
   Name->SetBinContent(42,4);
   Name->SetBinContent(43,1);
   Name->SetBinContent(44,2);
   Name->SetBinContent(47,1);
   Name->SetBinContent(51,3);
   Name->SetEntries(6998);
   Name->SetStats(0);
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,16);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(127.2405);
   Gaussian->SetNDF(37);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,563.5863);
   Gaussian->SetParError(0,8.540985);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,7.41887);
   Gaussian->SetParError(1,0.01998804);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,1.555812);
   Gaussian->SetParError(2,0.01428014);
   Gaussian->SetParLimits(2,0,16.12294);
   Name->GetListOfFunctions()->Add(Gaussian);
   Name->SetLineWidth(2);
   Name->SetMarkerStyle(20);
   Name->GetXaxis()->SetTitle("PFO Energy [GeV]");
   Name->GetXaxis()->SetNdivisions(505);
   Name->GetXaxis()->SetLabelFont(132);
   Name->GetXaxis()->SetLabelSize(0.03);
   Name->GetXaxis()->SetTitleSize(0.036);
   Name->GetXaxis()->SetTitleFont(132);
   Name->GetYaxis()->SetTitle("Entries");
   Name->GetYaxis()->SetLabelFont(132);
   Name->GetYaxis()->SetLabelSize(0.03);
   Name->GetYaxis()->SetTitleSize(0.036);
   Name->GetYaxis()->SetTitleFont(132);
   Name->GetZaxis()->SetLabelFont(132);
   Name->GetZaxis()->SetLabelSize(0.03);
   Name->GetZaxis()->SetTitleSize(0.036);
   Name->GetZaxis()->SetTitleFont(132);
   Name->Draw("");
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,50);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(127.2405);
   Gaussian->SetNDF(37);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,563.5863);
   Gaussian->SetParError(0,8.540985);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,7.41887);
   Gaussian->SetParError(1,0.01998804);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,1.555812);
   Gaussian->SetParError(2,0.01428014);
   Gaussian->SetParLimits(2,0,16.12294);
   Gaussian->Draw("same");
   
   TPaveText *pt = new TPaveText(0.01,0.945,0.1004698,0.995,"blNDC");
   pt->SetName("title");
   pt->SetBorderSize(1);
   pt->SetFillColor(0);
   pt->SetLineWidth(2);
   pt->SetTextFont(132);
   TText *text = pt->AddText("Title");
   pt->Draw();
   Name->Modified();
   Name->cd();
   Name->SetSelected(Name);
}
