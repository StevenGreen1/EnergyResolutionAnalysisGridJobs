{
//=========Macro generated from canvas: Name/Title
//=========  (Tue Oct 20 18:45:40 2015) by ROOT version5.34/30
   TCanvas *Name = new TCanvas("Name", "Title",0,0,600,500);
   gStyle->SetOptStat(0);
   Name->SetHighLightColor(2);
   Name->Range(-0.7500001,-58.8,6.75,529.2);
   Name->SetFillColor(0);
   Name->SetBorderMode(0);
   Name->SetBorderSize(2);
   Name->SetTickx(1);
   Name->SetTicky(1);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   
   TH1F *Name = new TH1F("Name","Title",50,0,6);
   Name->SetBinContent(3,9);
   Name->SetBinContent(4,22);
   Name->SetBinContent(5,29);
   Name->SetBinContent(6,41);
   Name->SetBinContent(7,52);
   Name->SetBinContent(8,53);
   Name->SetBinContent(9,72);
   Name->SetBinContent(10,105);
   Name->SetBinContent(11,130);
   Name->SetBinContent(12,188);
   Name->SetBinContent(13,201);
   Name->SetBinContent(14,274);
   Name->SetBinContent(15,319);
   Name->SetBinContent(16,359);
   Name->SetBinContent(17,395);
   Name->SetBinContent(18,432);
   Name->SetBinContent(19,420);
   Name->SetBinContent(20,448);
   Name->SetBinContent(21,444);
   Name->SetBinContent(22,422);
   Name->SetBinContent(23,401);
   Name->SetBinContent(24,371);
   Name->SetBinContent(25,365);
   Name->SetBinContent(26,303);
   Name->SetBinContent(27,260);
   Name->SetBinContent(28,237);
   Name->SetBinContent(29,221);
   Name->SetBinContent(30,172);
   Name->SetBinContent(31,150);
   Name->SetBinContent(32,135);
   Name->SetBinContent(33,114);
   Name->SetBinContent(34,100);
   Name->SetBinContent(35,54);
   Name->SetBinContent(36,46);
   Name->SetBinContent(37,42);
   Name->SetBinContent(38,31);
   Name->SetBinContent(39,27);
   Name->SetBinContent(40,23);
   Name->SetBinContent(41,17);
   Name->SetBinContent(42,17);
   Name->SetBinContent(43,9);
   Name->SetBinContent(44,13);
   Name->SetBinContent(45,10);
   Name->SetBinContent(46,6);
   Name->SetBinContent(47,8);
   Name->SetBinContent(48,5);
   Name->SetBinContent(49,2);
   Name->SetBinContent(50,5);
   Name->SetBinContent(51,19);
   Name->SetEntries(7578);
   Name->SetStats(0);
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,6);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(137.0057);
   Gaussian->SetNDF(45);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,424.793);
   Gaussian->SetParError(0,6.446702);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,2.488388);
   Gaussian->SetParError(1,0.01010001);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,0.8395381);
   Gaussian->SetParError(2,0.008435086);
   Gaussian->SetParLimits(2,0,8.659569);
   Name->GetListOfFunctions()->Add(Gaussian);
   Name->SetLineWidth(2);
   Name->SetMarkerStyle(20);
   Name->GetXaxis()->SetTitle("PFO Energy [GeV]");
   Name->GetXaxis()->SetNdivisions(505);
   Name->GetXaxis()->SetLabelFont(132);
   Name->GetXaxis()->SetLabelSize(0.03);
   Name->GetXaxis()->SetTitleSize(0.036);
   Name->GetXaxis()->SetTitleFont(132);
   Name->GetYaxis()->SetTitle("Entries");
   Name->GetYaxis()->SetLabelFont(132);
   Name->GetYaxis()->SetLabelSize(0.03);
   Name->GetYaxis()->SetTitleSize(0.036);
   Name->GetYaxis()->SetTitleFont(132);
   Name->GetZaxis()->SetLabelFont(132);
   Name->GetZaxis()->SetLabelSize(0.03);
   Name->GetZaxis()->SetTitleSize(0.036);
   Name->GetZaxis()->SetTitleFont(132);
   Name->Draw("");
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,50);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(137.0057);
   Gaussian->SetNDF(45);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,424.793);
   Gaussian->SetParError(0,6.446702);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,2.488388);
   Gaussian->SetParError(1,0.01010001);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,0.8395381);
   Gaussian->SetParError(2,0.008435086);
   Gaussian->SetParLimits(2,0,8.659569);
   Gaussian->Draw("same");
   
   TPaveText *pt = new TPaveText(0.01,0.945,0.1004698,0.995,"blNDC");
   pt->SetName("title");
   pt->SetBorderSize(1);
   pt->SetFillColor(0);
   pt->SetLineWidth(2);
   pt->SetTextFont(132);
   TText *text = pt->AddText("Title");
   pt->Draw();
   Name->Modified();
   Name->cd();
   Name->SetSelected(Name);
}
