{
//=========Macro generated from canvas: Name/Title
//=========  (Tue Oct 20 18:45:35 2015) by ROOT version5.34/30
   TCanvas *Name = new TCanvas("Name", "Title",0,0,600,500);
   gStyle->SetOptStat(0);
   Name->SetHighLightColor(2);
   Name->Range(-3.75,-104.6063,33.75,941.4563);
   Name->SetFillColor(0);
   Name->SetBorderMode(0);
   Name->SetBorderSize(2);
   Name->SetTickx(1);
   Name->SetTicky(1);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   
   TH1F *Name = new TH1F("Name","Title",50,0,30);
   Name->SetBinContent(6,1);
   Name->SetBinContent(7,1);
   Name->SetBinContent(9,4);
   Name->SetBinContent(10,1);
   Name->SetBinContent(11,5);
   Name->SetBinContent(12,7);
   Name->SetBinContent(13,9);
   Name->SetBinContent(14,18);
   Name->SetBinContent(15,17);
   Name->SetBinContent(16,43);
   Name->SetBinContent(17,78);
   Name->SetBinContent(18,140);
   Name->SetBinContent(19,196);
   Name->SetBinContent(20,343);
   Name->SetBinContent(21,483);
   Name->SetBinContent(22,674);
   Name->SetBinContent(23,723);
   Name->SetBinContent(24,797);
   Name->SetBinContent(25,784);
   Name->SetBinContent(26,636);
   Name->SetBinContent(27,609);
   Name->SetBinContent(28,493);
   Name->SetBinContent(29,365);
   Name->SetBinContent(30,264);
   Name->SetBinContent(31,165);
   Name->SetBinContent(32,111);
   Name->SetBinContent(33,63);
   Name->SetBinContent(34,41);
   Name->SetBinContent(35,23);
   Name->SetBinContent(36,11);
   Name->SetBinContent(37,10);
   Name->SetBinContent(38,3);
   Name->SetBinContent(39,2);
   Name->SetBinContent(40,1);
   Name->SetBinContent(41,2);
   Name->SetBinContent(45,1);
   Name->SetEntries(7124);
   Name->SetStats(0);
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,30);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(77.73861);
   Gaussian->SetNDF(33);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,764.7785);
   Gaussian->SetParError(0,11.61535);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,14.49973);
   Gaussian->SetParError(1,0.02656715);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,2.205396);
   Gaussian->SetParError(2,0.02077593);
   Gaussian->SetParLimits(2,0,22.77542);
   Name->GetListOfFunctions()->Add(Gaussian);
   Name->SetLineWidth(2);
   Name->SetMarkerStyle(20);
   Name->GetXaxis()->SetTitle("PFO Energy [GeV]");
   Name->GetXaxis()->SetNdivisions(505);
   Name->GetXaxis()->SetLabelFont(132);
   Name->GetXaxis()->SetLabelSize(0.03);
   Name->GetXaxis()->SetTitleSize(0.036);
   Name->GetXaxis()->SetTitleFont(132);
   Name->GetYaxis()->SetTitle("Entries");
   Name->GetYaxis()->SetLabelFont(132);
   Name->GetYaxis()->SetLabelSize(0.03);
   Name->GetYaxis()->SetTitleSize(0.036);
   Name->GetYaxis()->SetTitleFont(132);
   Name->GetZaxis()->SetLabelFont(132);
   Name->GetZaxis()->SetLabelSize(0.03);
   Name->GetZaxis()->SetTitleSize(0.036);
   Name->GetZaxis()->SetTitleFont(132);
   Name->Draw("");
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,50);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(77.73861);
   Gaussian->SetNDF(33);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,764.7785);
   Gaussian->SetParError(0,11.61535);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,14.49973);
   Gaussian->SetParError(1,0.02656715);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,2.205396);
   Gaussian->SetParError(2,0.02077593);
   Gaussian->SetParLimits(2,0,22.77542);
   Gaussian->Draw("same");
   
   TPaveText *pt = new TPaveText(0.01,0.945,0.1004698,0.995,"blNDC");
   pt->SetName("title");
   pt->SetBorderSize(1);
   pt->SetFillColor(0);
   pt->SetLineWidth(2);
   pt->SetTextFont(132);
   TText *text = pt->AddText("Title");
   pt->Draw();
   Name->Modified();
   Name->cd();
   Name->SetSelected(Name);
}
