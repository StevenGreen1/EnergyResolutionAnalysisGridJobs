{
//=========Macro generated from canvas: Name/Title
//=========  (Tue Oct 20 18:44:47 2015) by ROOT version5.34/30
   TCanvas *Name = new TCanvas("Name", "Title",0,0,600,500);
   gStyle->SetOptStat(0);
   Name->SetHighLightColor(2);
   Name->Range(-2,-73.36876,18,660.3188);
   Name->SetFillColor(0);
   Name->SetBorderMode(0);
   Name->SetBorderSize(2);
   Name->SetTickx(1);
   Name->SetTicky(1);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   
   TH1F *Name = new TH1F("Name","Title",50,0,16);
   Name->SetBinContent(5,3);
   Name->SetBinContent(6,4);
   Name->SetBinContent(7,8);
   Name->SetBinContent(8,3);
   Name->SetBinContent(9,8);
   Name->SetBinContent(10,17);
   Name->SetBinContent(11,29);
   Name->SetBinContent(12,46);
   Name->SetBinContent(13,80);
   Name->SetBinContent(14,109);
   Name->SetBinContent(15,184);
   Name->SetBinContent(16,268);
   Name->SetBinContent(17,354);
   Name->SetBinContent(18,438);
   Name->SetBinContent(19,465);
   Name->SetBinContent(20,509);
   Name->SetBinContent(21,536);
   Name->SetBinContent(22,559);
   Name->SetBinContent(23,515);
   Name->SetBinContent(24,482);
   Name->SetBinContent(25,426);
   Name->SetBinContent(26,410);
   Name->SetBinContent(27,347);
   Name->SetBinContent(28,280);
   Name->SetBinContent(29,234);
   Name->SetBinContent(30,201);
   Name->SetBinContent(31,139);
   Name->SetBinContent(32,126);
   Name->SetBinContent(33,67);
   Name->SetBinContent(34,56);
   Name->SetBinContent(35,60);
   Name->SetBinContent(36,35);
   Name->SetBinContent(37,33);
   Name->SetBinContent(38,12);
   Name->SetBinContent(39,16);
   Name->SetBinContent(40,18);
   Name->SetBinContent(41,7);
   Name->SetBinContent(42,8);
   Name->SetBinContent(43,1);
   Name->SetBinContent(44,5);
   Name->SetBinContent(45,2);
   Name->SetBinContent(46,1);
   Name->SetBinContent(51,4);
   Name->SetEntries(7105);
   Name->SetStats(0);
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,16);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(196.3832);
   Gaussian->SetNDF(39);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,540.2802);
   Gaussian->SetParError(0,8.224859);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,7.095091);
   Gaussian->SetParError(1,0.02127182);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,1.63177);
   Gaussian->SetParError(2,0.01524105);
   Gaussian->SetParLimits(2,0,17.42155);
   Name->GetListOfFunctions()->Add(Gaussian);
   Name->SetLineWidth(2);
   Name->SetMarkerStyle(20);
   Name->GetXaxis()->SetTitle("PFO Energy [GeV]");
   Name->GetXaxis()->SetNdivisions(505);
   Name->GetXaxis()->SetLabelFont(132);
   Name->GetXaxis()->SetLabelSize(0.03);
   Name->GetXaxis()->SetTitleSize(0.036);
   Name->GetXaxis()->SetTitleFont(132);
   Name->GetYaxis()->SetTitle("Entries");
   Name->GetYaxis()->SetLabelFont(132);
   Name->GetYaxis()->SetLabelSize(0.03);
   Name->GetYaxis()->SetTitleSize(0.036);
   Name->GetYaxis()->SetTitleFont(132);
   Name->GetZaxis()->SetLabelFont(132);
   Name->GetZaxis()->SetLabelSize(0.03);
   Name->GetZaxis()->SetTitleSize(0.036);
   Name->GetZaxis()->SetTitleFont(132);
   Name->Draw("");
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,50);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(196.3832);
   Gaussian->SetNDF(39);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,540.2802);
   Gaussian->SetParError(0,8.224859);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,7.095091);
   Gaussian->SetParError(1,0.02127182);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,1.63177);
   Gaussian->SetParError(2,0.01524105);
   Gaussian->SetParLimits(2,0,17.42155);
   Gaussian->Draw("same");
   
   TPaveText *pt = new TPaveText(0.01,0.945,0.1004698,0.995,"blNDC");
   pt->SetName("title");
   pt->SetBorderSize(1);
   pt->SetFillColor(0);
   pt->SetLineWidth(2);
   pt->SetTextFont(132);
   TText *text = pt->AddText("Title");
   pt->Draw();
   Name->Modified();
   Name->cd();
   Name->SetSelected(Name);
}
