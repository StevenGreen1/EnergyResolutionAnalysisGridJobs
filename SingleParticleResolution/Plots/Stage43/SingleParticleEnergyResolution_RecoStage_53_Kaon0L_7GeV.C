{
//=========Macro generated from canvas: Name/Title
//=========  (Tue Oct 20 18:45:33 2015) by ROOT version5.34/30
   TCanvas *Name = new TCanvas("Name", "Title",0,0,600,500);
   gStyle->SetOptStat(0);
   Name->SetHighLightColor(2);
   Name->Range(-1.75,-65.8875,15.75,592.9875);
   Name->SetFillColor(0);
   Name->SetBorderMode(0);
   Name->SetBorderSize(2);
   Name->SetTickx(1);
   Name->SetTicky(1);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   
   TH1F *Name = new TH1F("Name","Title",50,0,14);
   Name->SetBinContent(2,1);
   Name->SetBinContent(3,2);
   Name->SetBinContent(5,6);
   Name->SetBinContent(6,4);
   Name->SetBinContent(7,2);
   Name->SetBinContent(8,1);
   Name->SetBinContent(9,6);
   Name->SetBinContent(10,16);
   Name->SetBinContent(11,17);
   Name->SetBinContent(12,31);
   Name->SetBinContent(13,58);
   Name->SetBinContent(14,76);
   Name->SetBinContent(15,142);
   Name->SetBinContent(16,178);
   Name->SetBinContent(17,266);
   Name->SetBinContent(18,322);
   Name->SetBinContent(19,340);
   Name->SetBinContent(20,423);
   Name->SetBinContent(21,476);
   Name->SetBinContent(22,473);
   Name->SetBinContent(23,502);
   Name->SetBinContent(24,457);
   Name->SetBinContent(25,463);
   Name->SetBinContent(26,378);
   Name->SetBinContent(27,367);
   Name->SetBinContent(28,290);
   Name->SetBinContent(29,250);
   Name->SetBinContent(30,181);
   Name->SetBinContent(31,181);
   Name->SetBinContent(32,122);
   Name->SetBinContent(33,74);
   Name->SetBinContent(34,77);
   Name->SetBinContent(35,60);
   Name->SetBinContent(36,33);
   Name->SetBinContent(37,25);
   Name->SetBinContent(38,16);
   Name->SetBinContent(39,15);
   Name->SetBinContent(40,16);
   Name->SetBinContent(41,3);
   Name->SetBinContent(42,4);
   Name->SetBinContent(43,2);
   Name->SetBinContent(44,1);
   Name->SetBinContent(45,2);
   Name->SetBinContent(46,1);
   Name->SetBinContent(47,1);
   Name->SetBinContent(48,1);
   Name->SetBinContent(51,2);
   Name->SetEntries(6364);
   Name->SetStats(0);
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,14);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(137.197);
   Gaussian->SetNDF(43);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,487.1485);
   Gaussian->SetParError(0,7.6811);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,6.412817);
   Gaussian->SetParError(1,0.01928899);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,1.427445);
   Gaussian->SetParError(2,0.01339541);
   Gaussian->SetParLimits(2,0,15.11132);
   Name->GetListOfFunctions()->Add(Gaussian);
   Name->SetLineWidth(2);
   Name->SetMarkerStyle(20);
   Name->GetXaxis()->SetTitle("PFO Energy [GeV]");
   Name->GetXaxis()->SetNdivisions(505);
   Name->GetXaxis()->SetLabelFont(132);
   Name->GetXaxis()->SetLabelSize(0.03);
   Name->GetXaxis()->SetTitleSize(0.036);
   Name->GetXaxis()->SetTitleFont(132);
   Name->GetYaxis()->SetTitle("Entries");
   Name->GetYaxis()->SetLabelFont(132);
   Name->GetYaxis()->SetLabelSize(0.03);
   Name->GetYaxis()->SetTitleSize(0.036);
   Name->GetYaxis()->SetTitleFont(132);
   Name->GetZaxis()->SetLabelFont(132);
   Name->GetZaxis()->SetLabelSize(0.03);
   Name->GetZaxis()->SetTitleSize(0.036);
   Name->GetZaxis()->SetTitleFont(132);
   Name->Draw("");
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,50);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(137.197);
   Gaussian->SetNDF(43);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,487.1485);
   Gaussian->SetParError(0,7.6811);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,6.412817);
   Gaussian->SetParError(1,0.01928899);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,1.427445);
   Gaussian->SetParError(2,0.01339541);
   Gaussian->SetParLimits(2,0,15.11132);
   Gaussian->Draw("same");
   
   TPaveText *pt = new TPaveText(0.01,0.945,0.1004698,0.995,"blNDC");
   pt->SetName("title");
   pt->SetBorderSize(1);
   pt->SetFillColor(0);
   pt->SetLineWidth(2);
   pt->SetTextFont(132);
   TText *text = pt->AddText("Title");
   pt->Draw();
   Name->Modified();
   Name->cd();
   Name->SetSelected(Name);
}
