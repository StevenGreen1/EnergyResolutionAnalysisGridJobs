{
//=========Macro generated from canvas: Name/Title
//=========  (Tue Oct 20 18:45:27 2015) by ROOT version5.34/30
   TCanvas *Name = new TCanvas("Name", "Title",0,0,600,500);
   gStyle->SetOptStat(0);
   Name->SetHighLightColor(2);
   Name->Range(-2.25,-79.93126,20.25,719.3813);
   Name->SetFillColor(0);
   Name->SetBorderMode(0);
   Name->SetBorderSize(2);
   Name->SetTickx(1);
   Name->SetTicky(1);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   
   TH1F *Name = new TH1F("Name","Title",50,0,18);
   Name->SetBinContent(6,3);
   Name->SetBinContent(7,2);
   Name->SetBinContent(8,5);
   Name->SetBinContent(9,9);
   Name->SetBinContent(10,3);
   Name->SetBinContent(11,11);
   Name->SetBinContent(12,16);
   Name->SetBinContent(13,25);
   Name->SetBinContent(14,38);
   Name->SetBinContent(15,99);
   Name->SetBinContent(16,124);
   Name->SetBinContent(17,222);
   Name->SetBinContent(18,283);
   Name->SetBinContent(19,329);
   Name->SetBinContent(20,441);
   Name->SetBinContent(21,514);
   Name->SetBinContent(22,559);
   Name->SetBinContent(23,555);
   Name->SetBinContent(24,609);
   Name->SetBinContent(25,587);
   Name->SetBinContent(26,494);
   Name->SetBinContent(27,452);
   Name->SetBinContent(28,400);
   Name->SetBinContent(29,323);
   Name->SetBinContent(30,218);
   Name->SetBinContent(31,173);
   Name->SetBinContent(32,138);
   Name->SetBinContent(33,87);
   Name->SetBinContent(34,60);
   Name->SetBinContent(35,44);
   Name->SetBinContent(36,23);
   Name->SetBinContent(37,25);
   Name->SetBinContent(38,16);
   Name->SetBinContent(39,13);
   Name->SetBinContent(40,3);
   Name->SetBinContent(42,3);
   Name->SetBinContent(43,3);
   Name->SetBinContent(44,1);
   Name->SetBinContent(45,2);
   Name->SetBinContent(46,1);
   Name->SetBinContent(48,1);
   Name->SetBinContent(49,1);
   Name->SetBinContent(51,1);
   Name->SetEntries(6916);
   Name->SetStats(0);
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,18);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(92.45753);
   Gaussian->SetNDF(39);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,590.5584);
   Gaussian->SetParError(0,8.9592);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,8.441783);
   Gaussian->SetParError(1,0.02056152);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,1.659394);
   Gaussian->SetParError(2,0.01519439);
   Gaussian->SetParLimits(2,0,17.32589);
   Name->GetListOfFunctions()->Add(Gaussian);
   Name->SetLineWidth(2);
   Name->SetMarkerStyle(20);
   Name->GetXaxis()->SetTitle("PFO Energy [GeV]");
   Name->GetXaxis()->SetNdivisions(505);
   Name->GetXaxis()->SetLabelFont(132);
   Name->GetXaxis()->SetLabelSize(0.03);
   Name->GetXaxis()->SetTitleSize(0.036);
   Name->GetXaxis()->SetTitleFont(132);
   Name->GetYaxis()->SetTitle("Entries");
   Name->GetYaxis()->SetLabelFont(132);
   Name->GetYaxis()->SetLabelSize(0.03);
   Name->GetYaxis()->SetTitleSize(0.036);
   Name->GetYaxis()->SetTitleFont(132);
   Name->GetZaxis()->SetLabelFont(132);
   Name->GetZaxis()->SetLabelSize(0.03);
   Name->GetZaxis()->SetTitleSize(0.036);
   Name->GetZaxis()->SetTitleFont(132);
   Name->Draw("");
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,50);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(92.45753);
   Gaussian->SetNDF(39);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,590.5584);
   Gaussian->SetParError(0,8.9592);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,8.441783);
   Gaussian->SetParError(1,0.02056152);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,1.659394);
   Gaussian->SetParError(2,0.01519439);
   Gaussian->SetParLimits(2,0,17.32589);
   Gaussian->Draw("same");
   
   TPaveText *pt = new TPaveText(0.01,0.945,0.1004698,0.995,"blNDC");
   pt->SetName("title");
   pt->SetBorderSize(1);
   pt->SetFillColor(0);
   pt->SetLineWidth(2);
   pt->SetTextFont(132);
   TText *text = pt->AddText("Title");
   pt->Draw();
   Name->Modified();
   Name->cd();
   Name->SetSelected(Name);
}
