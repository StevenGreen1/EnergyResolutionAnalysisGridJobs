{
//=========Macro generated from canvas: Name/Title
//=========  (Tue Oct 20 18:45:41 2015) by ROOT version5.34/30
   TCanvas *Name = new TCanvas("Name", "Title",0,0,600,500);
   gStyle->SetOptStat(0);
   Name->SetHighLightColor(2);
   Name->Range(-1.5,-72.18751,13.5,649.6875);
   Name->SetFillColor(0);
   Name->SetBorderMode(0);
   Name->SetBorderSize(2);
   Name->SetTickx(1);
   Name->SetTicky(1);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   
   TH1F *Name = new TH1F("Name","Title",50,0,12);
   Name->SetBinContent(2,2);
   Name->SetBinContent(3,1);
   Name->SetBinContent(4,1);
   Name->SetBinContent(5,1);
   Name->SetBinContent(6,4);
   Name->SetBinContent(7,12);
   Name->SetBinContent(8,14);
   Name->SetBinContent(9,20);
   Name->SetBinContent(10,19);
   Name->SetBinContent(11,36);
   Name->SetBinContent(12,78);
   Name->SetBinContent(13,100);
   Name->SetBinContent(14,154);
   Name->SetBinContent(15,194);
   Name->SetBinContent(16,274);
   Name->SetBinContent(17,364);
   Name->SetBinContent(18,430);
   Name->SetBinContent(19,500);
   Name->SetBinContent(20,550);
   Name->SetBinContent(21,534);
   Name->SetBinContent(22,522);
   Name->SetBinContent(23,530);
   Name->SetBinContent(24,479);
   Name->SetBinContent(25,451);
   Name->SetBinContent(26,349);
   Name->SetBinContent(27,308);
   Name->SetBinContent(28,256);
   Name->SetBinContent(29,241);
   Name->SetBinContent(30,183);
   Name->SetBinContent(31,165);
   Name->SetBinContent(32,109);
   Name->SetBinContent(33,78);
   Name->SetBinContent(34,82);
   Name->SetBinContent(35,46);
   Name->SetBinContent(36,44);
   Name->SetBinContent(37,26);
   Name->SetBinContent(38,26);
   Name->SetBinContent(39,16);
   Name->SetBinContent(40,9);
   Name->SetBinContent(41,6);
   Name->SetBinContent(42,6);
   Name->SetBinContent(43,3);
   Name->SetBinContent(44,2);
   Name->SetBinContent(45,2);
   Name->SetBinContent(46,2);
   Name->SetBinContent(47,2);
   Name->SetBinContent(49,1);
   Name->SetBinContent(50,1);
   Name->SetBinContent(51,5);
   Name->SetEntries(7238);
   Name->SetStats(0);
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,12);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(188.5297);
   Gaussian->SetNDF(45);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,524.9098);
   Gaussian->SetParError(0,8.058223);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,5.256522);
   Gaussian->SetParError(1,0.01657403);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,1.284999);
   Gaussian->SetParError(2,0.0124449);
   Gaussian->SetParLimits(2,0,13.57174);
   Name->GetListOfFunctions()->Add(Gaussian);
   Name->SetLineWidth(2);
   Name->SetMarkerStyle(20);
   Name->GetXaxis()->SetTitle("PFO Energy [GeV]");
   Name->GetXaxis()->SetNdivisions(505);
   Name->GetXaxis()->SetLabelFont(132);
   Name->GetXaxis()->SetLabelSize(0.03);
   Name->GetXaxis()->SetTitleSize(0.036);
   Name->GetXaxis()->SetTitleFont(132);
   Name->GetYaxis()->SetTitle("Entries");
   Name->GetYaxis()->SetLabelFont(132);
   Name->GetYaxis()->SetLabelSize(0.03);
   Name->GetYaxis()->SetTitleSize(0.036);
   Name->GetYaxis()->SetTitleFont(132);
   Name->GetZaxis()->SetLabelFont(132);
   Name->GetZaxis()->SetLabelSize(0.03);
   Name->GetZaxis()->SetTitleSize(0.036);
   Name->GetZaxis()->SetTitleFont(132);
   Name->Draw("");
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,50);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(188.5297);
   Gaussian->SetNDF(45);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,524.9098);
   Gaussian->SetParError(0,8.058223);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,5.256522);
   Gaussian->SetParError(1,0.01657403);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,1.284999);
   Gaussian->SetParError(2,0.0124449);
   Gaussian->SetParLimits(2,0,13.57174);
   Gaussian->Draw("same");
   
   TPaveText *pt = new TPaveText(0.01,0.945,0.1004698,0.995,"blNDC");
   pt->SetName("title");
   pt->SetBorderSize(1);
   pt->SetFillColor(0);
   pt->SetLineWidth(2);
   pt->SetTextFont(132);
   TText *text = pt->AddText("Title");
   pt->Draw();
   Name->Modified();
   Name->cd();
   Name->SetSelected(Name);
}
