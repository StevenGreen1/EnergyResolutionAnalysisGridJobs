{
//=========Macro generated from canvas: Name/Title
//=========  (Tue Oct 20 18:44:25 2015) by ROOT version5.34/30
   TCanvas *Name = new TCanvas("Name", "Title",0,0,600,500);
   gStyle->SetOptStat(0);
   Name->SetHighLightColor(2);
   Name->Range(-2,-71.92501,18,647.325);
   Name->SetFillColor(0);
   Name->SetBorderMode(0);
   Name->SetBorderSize(2);
   Name->SetTickx(1);
   Name->SetTicky(1);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   
   TH1F *Name = new TH1F("Name","Title",50,0,16);
   Name->SetBinContent(4,1);
   Name->SetBinContent(5,2);
   Name->SetBinContent(6,1);
   Name->SetBinContent(7,9);
   Name->SetBinContent(8,5);
   Name->SetBinContent(9,6);
   Name->SetBinContent(10,10);
   Name->SetBinContent(11,28);
   Name->SetBinContent(12,37);
   Name->SetBinContent(13,66);
   Name->SetBinContent(14,118);
   Name->SetBinContent(15,154);
   Name->SetBinContent(16,212);
   Name->SetBinContent(17,334);
   Name->SetBinContent(18,412);
   Name->SetBinContent(19,446);
   Name->SetBinContent(20,471);
   Name->SetBinContent(21,499);
   Name->SetBinContent(22,514);
   Name->SetBinContent(23,548);
   Name->SetBinContent(24,524);
   Name->SetBinContent(25,463);
   Name->SetBinContent(26,445);
   Name->SetBinContent(27,372);
   Name->SetBinContent(28,338);
   Name->SetBinContent(29,249);
   Name->SetBinContent(30,196);
   Name->SetBinContent(31,160);
   Name->SetBinContent(32,135);
   Name->SetBinContent(33,97);
   Name->SetBinContent(34,85);
   Name->SetBinContent(35,53);
   Name->SetBinContent(36,36);
   Name->SetBinContent(37,36);
   Name->SetBinContent(38,11);
   Name->SetBinContent(39,11);
   Name->SetBinContent(40,8);
   Name->SetBinContent(41,5);
   Name->SetBinContent(42,5);
   Name->SetBinContent(43,1);
   Name->SetBinContent(44,2);
   Name->SetBinContent(51,3);
   Name->SetEntries(7108);
   Name->SetStats(0);
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,16);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(138.9406);
   Gaussian->SetNDF(38);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,539.9799);
   Gaussian->SetParError(0,8.081581);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,7.244098);
   Gaussian->SetParError(1,0.02100664);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,1.647044);
   Gaussian->SetParError(2,0.01478691);
   Gaussian->SetParLimits(2,0,17.06824);
   Name->GetListOfFunctions()->Add(Gaussian);
   Name->SetLineWidth(2);
   Name->SetMarkerStyle(20);
   Name->GetXaxis()->SetTitle("PFO Energy [GeV]");
   Name->GetXaxis()->SetNdivisions(505);
   Name->GetXaxis()->SetLabelFont(132);
   Name->GetXaxis()->SetLabelSize(0.03);
   Name->GetXaxis()->SetTitleSize(0.036);
   Name->GetXaxis()->SetTitleFont(132);
   Name->GetYaxis()->SetTitle("Entries");
   Name->GetYaxis()->SetLabelFont(132);
   Name->GetYaxis()->SetLabelSize(0.03);
   Name->GetYaxis()->SetTitleSize(0.036);
   Name->GetYaxis()->SetTitleFont(132);
   Name->GetZaxis()->SetLabelFont(132);
   Name->GetZaxis()->SetLabelSize(0.03);
   Name->GetZaxis()->SetTitleSize(0.036);
   Name->GetZaxis()->SetTitleFont(132);
   Name->Draw("");
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,50);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(138.9406);
   Gaussian->SetNDF(38);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,539.9799);
   Gaussian->SetParError(0,8.081581);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,7.244098);
   Gaussian->SetParError(1,0.02100664);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,1.647044);
   Gaussian->SetParError(2,0.01478691);
   Gaussian->SetParLimits(2,0,17.06824);
   Gaussian->Draw("same");
   
   TPaveText *pt = new TPaveText(0.01,0.945,0.1004698,0.995,"blNDC");
   pt->SetName("title");
   pt->SetBorderSize(1);
   pt->SetFillColor(0);
   pt->SetLineWidth(2);
   pt->SetTextFont(132);
   TText *text = pt->AddText("Title");
   pt->Draw();
   Name->Modified();
   Name->cd();
   Name->SetSelected(Name);
}
