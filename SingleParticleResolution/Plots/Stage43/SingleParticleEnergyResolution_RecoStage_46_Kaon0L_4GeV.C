{
//=========Macro generated from canvas: Name/Title
//=========  (Tue Oct 20 18:44:38 2015) by ROOT version5.34/30
   TCanvas *Name = new TCanvas("Name", "Title",0,0,600,500);
   gStyle->SetOptStat(0);
   Name->SetHighLightColor(2);
   Name->Range(-1,-66.15,9,595.35);
   Name->SetFillColor(0);
   Name->SetBorderMode(0);
   Name->SetBorderSize(2);
   Name->SetTickx(1);
   Name->SetTicky(1);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   
   TH1F *Name = new TH1F("Name","Title",50,0,8);
   Name->SetBinContent(2,2);
   Name->SetBinContent(3,11);
   Name->SetBinContent(4,19);
   Name->SetBinContent(5,17);
   Name->SetBinContent(6,18);
   Name->SetBinContent(7,34);
   Name->SetBinContent(8,52);
   Name->SetBinContent(9,75);
   Name->SetBinContent(10,90);
   Name->SetBinContent(11,125);
   Name->SetBinContent(12,195);
   Name->SetBinContent(13,229);
   Name->SetBinContent(14,282);
   Name->SetBinContent(15,351);
   Name->SetBinContent(16,377);
   Name->SetBinContent(17,432);
   Name->SetBinContent(18,452);
   Name->SetBinContent(19,504);
   Name->SetBinContent(20,487);
   Name->SetBinContent(21,482);
   Name->SetBinContent(22,443);
   Name->SetBinContent(23,451);
   Name->SetBinContent(24,383);
   Name->SetBinContent(25,340);
   Name->SetBinContent(26,296);
   Name->SetBinContent(27,291);
   Name->SetBinContent(28,220);
   Name->SetBinContent(29,224);
   Name->SetBinContent(30,168);
   Name->SetBinContent(31,119);
   Name->SetBinContent(32,102);
   Name->SetBinContent(33,86);
   Name->SetBinContent(34,69);
   Name->SetBinContent(35,54);
   Name->SetBinContent(36,49);
   Name->SetBinContent(37,34);
   Name->SetBinContent(38,29);
   Name->SetBinContent(39,22);
   Name->SetBinContent(40,11);
   Name->SetBinContent(41,11);
   Name->SetBinContent(42,9);
   Name->SetBinContent(43,6);
   Name->SetBinContent(44,3);
   Name->SetBinContent(45,8);
   Name->SetBinContent(46,6);
   Name->SetBinContent(47,3);
   Name->SetBinContent(48,3);
   Name->SetBinContent(49,2);
   Name->SetBinContent(51,9);
   Name->SetEntries(7685);
   Name->SetStats(0);
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,8);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(156.0591);
   Gaussian->SetNDF(45);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,471.6331);
   Gaussian->SetParError(0,6.99324);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,3.271799);
   Gaussian->SetParError(1,0.01242428);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,1.018889);
   Gaussian->SetParError(2,0.009590953);
   Gaussian->SetParLimits(2,0,10.65381);
   Name->GetListOfFunctions()->Add(Gaussian);
   Name->SetLineWidth(2);
   Name->SetMarkerStyle(20);
   Name->GetXaxis()->SetTitle("PFO Energy [GeV]");
   Name->GetXaxis()->SetNdivisions(505);
   Name->GetXaxis()->SetLabelFont(132);
   Name->GetXaxis()->SetLabelSize(0.03);
   Name->GetXaxis()->SetTitleSize(0.036);
   Name->GetXaxis()->SetTitleFont(132);
   Name->GetYaxis()->SetTitle("Entries");
   Name->GetYaxis()->SetLabelFont(132);
   Name->GetYaxis()->SetLabelSize(0.03);
   Name->GetYaxis()->SetTitleSize(0.036);
   Name->GetYaxis()->SetTitleFont(132);
   Name->GetZaxis()->SetLabelFont(132);
   Name->GetZaxis()->SetLabelSize(0.03);
   Name->GetZaxis()->SetTitleSize(0.036);
   Name->GetZaxis()->SetTitleFont(132);
   Name->Draw("");
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,50);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(156.0591);
   Gaussian->SetNDF(45);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,471.6331);
   Gaussian->SetParError(0,6.99324);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,3.271799);
   Gaussian->SetParError(1,0.01242428);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,1.018889);
   Gaussian->SetParError(2,0.009590953);
   Gaussian->SetParLimits(2,0,10.65381);
   Gaussian->Draw("same");
   
   TPaveText *pt = new TPaveText(0.01,0.945,0.1004698,0.995,"blNDC");
   pt->SetName("title");
   pt->SetBorderSize(1);
   pt->SetFillColor(0);
   pt->SetLineWidth(2);
   pt->SetTextFont(132);
   TText *text = pt->AddText("Title");
   pt->Draw();
   Name->Modified();
   Name->cd();
   Name->SetSelected(Name);
}
