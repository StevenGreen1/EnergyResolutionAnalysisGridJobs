{
//=========Macro generated from canvas: Name/Title
//=========  (Tue Oct 20 18:45:08 2015) by ROOT version5.34/30
   TCanvas *Name = new TCanvas("Name", "Title",0,0,600,500);
   gStyle->SetOptStat(0);
   Name->SetHighLightColor(2);
   Name->Range(-0.25,-32.2875,2.25,290.5875);
   Name->SetFillColor(0);
   Name->SetBorderMode(0);
   Name->SetBorderSize(2);
   Name->SetTickx(1);
   Name->SetTicky(1);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   
   TH1F *Name = new TH1F("Name","Title",50,0,2);
   Name->SetBinContent(7,81);
   Name->SetBinContent(8,92);
   Name->SetBinContent(9,105);
   Name->SetBinContent(10,138);
   Name->SetBinContent(11,150);
   Name->SetBinContent(12,167);
   Name->SetBinContent(13,151);
   Name->SetBinContent(14,190);
   Name->SetBinContent(15,208);
   Name->SetBinContent(16,222);
   Name->SetBinContent(17,207);
   Name->SetBinContent(18,189);
   Name->SetBinContent(19,213);
   Name->SetBinContent(20,246);
   Name->SetBinContent(21,216);
   Name->SetBinContent(22,208);
   Name->SetBinContent(23,189);
   Name->SetBinContent(24,175);
   Name->SetBinContent(25,178);
   Name->SetBinContent(26,161);
   Name->SetBinContent(27,166);
   Name->SetBinContent(28,162);
   Name->SetBinContent(29,140);
   Name->SetBinContent(30,128);
   Name->SetBinContent(31,107);
   Name->SetBinContent(32,86);
   Name->SetBinContent(33,96);
   Name->SetBinContent(34,75);
   Name->SetBinContent(35,68);
   Name->SetBinContent(36,62);
   Name->SetBinContent(37,69);
   Name->SetBinContent(38,66);
   Name->SetBinContent(39,50);
   Name->SetBinContent(40,48);
   Name->SetBinContent(41,29);
   Name->SetBinContent(42,28);
   Name->SetBinContent(43,35);
   Name->SetBinContent(44,43);
   Name->SetBinContent(45,25);
   Name->SetBinContent(46,25);
   Name->SetBinContent(47,24);
   Name->SetBinContent(48,13);
   Name->SetBinContent(49,8);
   Name->SetBinContent(50,18);
   Name->SetBinContent(51,162);
   Name->SetEntries(5219);
   Name->SetStats(0);
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,2);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(105.5758);
   Gaussian->SetNDF(41);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,198.1211);
   Gaussian->SetParError(0,3.761217);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,0.7804341);
   Gaussian->SetParError(1,0.009292196);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,0.4532058);
   Gaussian->SetParError(2,0.009217294);
   Gaussian->SetParLimits(2,0,3.757109);
   Name->GetListOfFunctions()->Add(Gaussian);
   Name->SetLineWidth(2);
   Name->SetMarkerStyle(20);
   Name->GetXaxis()->SetTitle("PFO Energy [GeV]");
   Name->GetXaxis()->SetNdivisions(505);
   Name->GetXaxis()->SetLabelFont(132);
   Name->GetXaxis()->SetLabelSize(0.03);
   Name->GetXaxis()->SetTitleSize(0.036);
   Name->GetXaxis()->SetTitleFont(132);
   Name->GetYaxis()->SetTitle("Entries");
   Name->GetYaxis()->SetLabelFont(132);
   Name->GetYaxis()->SetLabelSize(0.03);
   Name->GetYaxis()->SetTitleSize(0.036);
   Name->GetYaxis()->SetTitleFont(132);
   Name->GetZaxis()->SetLabelFont(132);
   Name->GetZaxis()->SetLabelSize(0.03);
   Name->GetZaxis()->SetTitleSize(0.036);
   Name->GetZaxis()->SetTitleFont(132);
   Name->Draw("");
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,50);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(105.5758);
   Gaussian->SetNDF(41);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,198.1211);
   Gaussian->SetParError(0,3.761217);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,0.7804341);
   Gaussian->SetParError(1,0.009292196);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,0.4532058);
   Gaussian->SetParError(2,0.009217294);
   Gaussian->SetParLimits(2,0,3.757109);
   Gaussian->Draw("same");
   
   TPaveText *pt = new TPaveText(0.01,0.945,0.1004698,0.995,"blNDC");
   pt->SetName("title");
   pt->SetBorderSize(1);
   pt->SetFillColor(0);
   pt->SetLineWidth(2);
   pt->SetTextFont(132);
   TText *text = pt->AddText("Title");
   pt->Draw();
   Name->Modified();
   Name->cd();
   Name->SetSelected(Name);
}
