{
//=========Macro generated from canvas: Name/Title
//=========  (Tue Oct 20 18:46:23 2015) by ROOT version5.34/30
   TCanvas *Name = new TCanvas("Name", "Title",0,0,600,500);
   gStyle->SetOptStat(0);
   Name->SetHighLightColor(2);
   Name->Range(-1.5,-71.66251,13.5,644.9625);
   Name->SetFillColor(0);
   Name->SetBorderMode(0);
   Name->SetBorderSize(2);
   Name->SetTickx(1);
   Name->SetTicky(1);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   
   TH1F *Name = new TH1F("Name","Title",50,0,12);
   Name->SetBinContent(2,2);
   Name->SetBinContent(4,2);
   Name->SetBinContent(5,1);
   Name->SetBinContent(6,4);
   Name->SetBinContent(7,12);
   Name->SetBinContent(8,18);
   Name->SetBinContent(9,15);
   Name->SetBinContent(10,21);
   Name->SetBinContent(11,45);
   Name->SetBinContent(12,83);
   Name->SetBinContent(13,106);
   Name->SetBinContent(14,151);
   Name->SetBinContent(15,207);
   Name->SetBinContent(16,285);
   Name->SetBinContent(17,375);
   Name->SetBinContent(18,456);
   Name->SetBinContent(19,501);
   Name->SetBinContent(20,536);
   Name->SetBinContent(21,546);
   Name->SetBinContent(22,495);
   Name->SetBinContent(23,506);
   Name->SetBinContent(24,476);
   Name->SetBinContent(25,424);
   Name->SetBinContent(26,351);
   Name->SetBinContent(27,312);
   Name->SetBinContent(28,280);
   Name->SetBinContent(29,209);
   Name->SetBinContent(30,162);
   Name->SetBinContent(31,144);
   Name->SetBinContent(32,111);
   Name->SetBinContent(33,93);
   Name->SetBinContent(34,64);
   Name->SetBinContent(35,60);
   Name->SetBinContent(36,38);
   Name->SetBinContent(37,51);
   Name->SetBinContent(38,23);
   Name->SetBinContent(39,15);
   Name->SetBinContent(40,12);
   Name->SetBinContent(41,8);
   Name->SetBinContent(42,6);
   Name->SetBinContent(43,8);
   Name->SetBinContent(44,4);
   Name->SetBinContent(45,3);
   Name->SetBinContent(46,4);
   Name->SetBinContent(47,1);
   Name->SetBinContent(49,1);
   Name->SetBinContent(51,7);
   Name->SetEntries(7234);
   Name->SetStats(0);
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,12);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(220.7112);
   Gaussian->SetNDF(43);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,519.3931);
   Gaussian->SetParError(0,8.045342);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,5.227389);
   Gaussian->SetParError(1,0.01690145);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,1.291786);
   Gaussian->SetParError(2,0.0127579);
   Gaussian->SetParLimits(2,0,13.88418);
   Name->GetListOfFunctions()->Add(Gaussian);
   Name->SetLineWidth(2);
   Name->SetMarkerStyle(20);
   Name->GetXaxis()->SetTitle("PFO Energy [GeV]");
   Name->GetXaxis()->SetNdivisions(505);
   Name->GetXaxis()->SetLabelFont(132);
   Name->GetXaxis()->SetLabelSize(0.03);
   Name->GetXaxis()->SetTitleSize(0.036);
   Name->GetXaxis()->SetTitleFont(132);
   Name->GetYaxis()->SetTitle("Entries");
   Name->GetYaxis()->SetLabelFont(132);
   Name->GetYaxis()->SetLabelSize(0.03);
   Name->GetYaxis()->SetTitleSize(0.036);
   Name->GetYaxis()->SetTitleFont(132);
   Name->GetZaxis()->SetLabelFont(132);
   Name->GetZaxis()->SetLabelSize(0.03);
   Name->GetZaxis()->SetTitleSize(0.036);
   Name->GetZaxis()->SetTitleFont(132);
   Name->Draw("");
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,50);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(220.7112);
   Gaussian->SetNDF(43);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,519.3931);
   Gaussian->SetParError(0,8.045342);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,5.227389);
   Gaussian->SetParError(1,0.01690145);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,1.291786);
   Gaussian->SetParError(2,0.0127579);
   Gaussian->SetParLimits(2,0,13.88418);
   Gaussian->Draw("same");
   
   TPaveText *pt = new TPaveText(0.01,0.945,0.1004698,0.995,"blNDC");
   pt->SetName("title");
   pt->SetBorderSize(1);
   pt->SetFillColor(0);
   pt->SetLineWidth(2);
   pt->SetTextFont(132);
   TText *text = pt->AddText("Title");
   pt->Draw();
   Name->Modified();
   Name->cd();
   Name->SetSelected(Name);
}
