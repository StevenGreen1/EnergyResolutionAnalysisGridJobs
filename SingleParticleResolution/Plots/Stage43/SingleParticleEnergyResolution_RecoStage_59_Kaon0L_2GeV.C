{
//=========Macro generated from canvas: Name/Title
//=========  (Tue Oct 20 18:46:21 2015) by ROOT version5.34/30
   TCanvas *Name = new TCanvas("Name", "Title",0,0,600,500);
   gStyle->SetOptStat(0);
   Name->SetHighLightColor(2);
   Name->Range(-0.5,-54.8625,4.5,493.7625);
   Name->SetFillColor(0);
   Name->SetBorderMode(0);
   Name->SetBorderSize(2);
   Name->SetTickx(1);
   Name->SetTicky(1);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   
   TH1F *Name = new TH1F("Name","Title",50,0,4);
   Name->SetBinContent(4,22);
   Name->SetBinContent(5,55);
   Name->SetBinContent(6,49);
   Name->SetBinContent(7,71);
   Name->SetBinContent(8,89);
   Name->SetBinContent(9,139);
   Name->SetBinContent(10,171);
   Name->SetBinContent(11,184);
   Name->SetBinContent(12,203);
   Name->SetBinContent(13,246);
   Name->SetBinContent(14,272);
   Name->SetBinContent(15,310);
   Name->SetBinContent(16,372);
   Name->SetBinContent(17,336);
   Name->SetBinContent(18,410);
   Name->SetBinContent(19,418);
   Name->SetBinContent(20,374);
   Name->SetBinContent(21,389);
   Name->SetBinContent(22,340);
   Name->SetBinContent(23,317);
   Name->SetBinContent(24,323);
   Name->SetBinContent(25,273);
   Name->SetBinContent(26,257);
   Name->SetBinContent(27,225);
   Name->SetBinContent(28,221);
   Name->SetBinContent(29,195);
   Name->SetBinContent(30,149);
   Name->SetBinContent(31,153);
   Name->SetBinContent(32,112);
   Name->SetBinContent(33,89);
   Name->SetBinContent(34,95);
   Name->SetBinContent(35,73);
   Name->SetBinContent(36,68);
   Name->SetBinContent(37,64);
   Name->SetBinContent(38,51);
   Name->SetBinContent(39,43);
   Name->SetBinContent(40,33);
   Name->SetBinContent(41,26);
   Name->SetBinContent(42,21);
   Name->SetBinContent(43,18);
   Name->SetBinContent(44,22);
   Name->SetBinContent(45,19);
   Name->SetBinContent(46,13);
   Name->SetBinContent(47,7);
   Name->SetBinContent(48,9);
   Name->SetBinContent(49,11);
   Name->SetBinContent(50,10);
   Name->SetBinContent(51,49);
   Name->SetEntries(7396);
   Name->SetStats(0);
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,4);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(219.0291);
   Gaussian->SetNDF(44);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,359.8566);
   Gaussian->SetParError(0,5.826379);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,1.607464);
   Gaussian->SetParError(1,0.008154409);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,0.6429476);
   Gaussian->SetParError(2,0.00772619);
   Gaussian->SetParLimits(2,0,6.512065);
   Name->GetListOfFunctions()->Add(Gaussian);
   Name->SetLineWidth(2);
   Name->SetMarkerStyle(20);
   Name->GetXaxis()->SetTitle("PFO Energy [GeV]");
   Name->GetXaxis()->SetNdivisions(505);
   Name->GetXaxis()->SetLabelFont(132);
   Name->GetXaxis()->SetLabelSize(0.03);
   Name->GetXaxis()->SetTitleSize(0.036);
   Name->GetXaxis()->SetTitleFont(132);
   Name->GetYaxis()->SetTitle("Entries");
   Name->GetYaxis()->SetLabelFont(132);
   Name->GetYaxis()->SetLabelSize(0.03);
   Name->GetYaxis()->SetTitleSize(0.036);
   Name->GetYaxis()->SetTitleFont(132);
   Name->GetZaxis()->SetLabelFont(132);
   Name->GetZaxis()->SetLabelSize(0.03);
   Name->GetZaxis()->SetTitleSize(0.036);
   Name->GetZaxis()->SetTitleFont(132);
   Name->Draw("");
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,50);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(219.0291);
   Gaussian->SetNDF(44);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,359.8566);
   Gaussian->SetParError(0,5.826379);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,1.607464);
   Gaussian->SetParError(1,0.008154409);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,0.6429476);
   Gaussian->SetParError(2,0.00772619);
   Gaussian->SetParLimits(2,0,6.512065);
   Gaussian->Draw("same");
   
   TPaveText *pt = new TPaveText(0.01,0.945,0.1004698,0.995,"blNDC");
   pt->SetName("title");
   pt->SetBorderSize(1);
   pt->SetFillColor(0);
   pt->SetLineWidth(2);
   pt->SetTextFont(132);
   TText *text = pt->AddText("Title");
   pt->Draw();
   Name->Modified();
   Name->cd();
   Name->SetSelected(Name);
}
