{
//=========Macro generated from canvas: Name/Title
//=========  (Tue Oct 20 18:45:49 2015) by ROOT version5.34/30
   TCanvas *Name = new TCanvas("Name", "Title",0,0,600,500);
   gStyle->SetOptStat(0);
   Name->SetHighLightColor(2);
   Name->Range(-1.75,-74.28751,15.75,668.5875);
   Name->SetFillColor(0);
   Name->SetBorderMode(0);
   Name->SetBorderSize(2);
   Name->SetTickx(1);
   Name->SetTicky(1);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   
   TH1F *Name = new TH1F("Name","Title",50,0,14);
   Name->SetBinContent(2,3);
   Name->SetBinContent(4,3);
   Name->SetBinContent(5,6);
   Name->SetBinContent(6,3);
   Name->SetBinContent(7,3);
   Name->SetBinContent(8,4);
   Name->SetBinContent(9,11);
   Name->SetBinContent(10,13);
   Name->SetBinContent(11,29);
   Name->SetBinContent(12,49);
   Name->SetBinContent(13,88);
   Name->SetBinContent(14,121);
   Name->SetBinContent(15,164);
   Name->SetBinContent(16,251);
   Name->SetBinContent(17,348);
   Name->SetBinContent(18,416);
   Name->SetBinContent(19,456);
   Name->SetBinContent(20,519);
   Name->SetBinContent(21,520);
   Name->SetBinContent(22,566);
   Name->SetBinContent(23,548);
   Name->SetBinContent(24,468);
   Name->SetBinContent(25,463);
   Name->SetBinContent(26,439);
   Name->SetBinContent(27,322);
   Name->SetBinContent(28,273);
   Name->SetBinContent(29,238);
   Name->SetBinContent(30,186);
   Name->SetBinContent(31,147);
   Name->SetBinContent(32,102);
   Name->SetBinContent(33,80);
   Name->SetBinContent(34,75);
   Name->SetBinContent(35,41);
   Name->SetBinContent(36,38);
   Name->SetBinContent(37,34);
   Name->SetBinContent(38,12);
   Name->SetBinContent(39,22);
   Name->SetBinContent(40,5);
   Name->SetBinContent(41,6);
   Name->SetBinContent(42,2);
   Name->SetBinContent(43,2);
   Name->SetBinContent(45,2);
   Name->SetBinContent(46,3);
   Name->SetBinContent(47,1);
   Name->SetBinContent(48,1);
   Name->SetBinContent(51,2);
   Name->SetEntries(7085);
   Name->SetStats(0);
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,14);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(166.0748);
   Gaussian->SetNDF(42);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,542.0899);
   Gaussian->SetParError(0,8.275608);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,6.2101);
   Gaussian->SetParError(1,0.01838166);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,1.425415);
   Gaussian->SetParError(2,0.01341839);
   Gaussian->SetParLimits(2,0,15.12236);
   Name->GetListOfFunctions()->Add(Gaussian);
   Name->SetLineWidth(2);
   Name->SetMarkerStyle(20);
   Name->GetXaxis()->SetTitle("PFO Energy [GeV]");
   Name->GetXaxis()->SetNdivisions(505);
   Name->GetXaxis()->SetLabelFont(132);
   Name->GetXaxis()->SetLabelSize(0.03);
   Name->GetXaxis()->SetTitleSize(0.036);
   Name->GetXaxis()->SetTitleFont(132);
   Name->GetYaxis()->SetTitle("Entries");
   Name->GetYaxis()->SetLabelFont(132);
   Name->GetYaxis()->SetLabelSize(0.03);
   Name->GetYaxis()->SetTitleSize(0.036);
   Name->GetYaxis()->SetTitleFont(132);
   Name->GetZaxis()->SetLabelFont(132);
   Name->GetZaxis()->SetLabelSize(0.03);
   Name->GetZaxis()->SetTitleSize(0.036);
   Name->GetZaxis()->SetTitleFont(132);
   Name->Draw("");
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,50);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(166.0748);
   Gaussian->SetNDF(42);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,542.0899);
   Gaussian->SetParError(0,8.275608);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,6.2101);
   Gaussian->SetParError(1,0.01838166);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,1.425415);
   Gaussian->SetParError(2,0.01341839);
   Gaussian->SetParLimits(2,0,15.12236);
   Gaussian->Draw("same");
   
   TPaveText *pt = new TPaveText(0.01,0.945,0.1004698,0.995,"blNDC");
   pt->SetName("title");
   pt->SetBorderSize(1);
   pt->SetFillColor(0);
   pt->SetLineWidth(2);
   pt->SetTextFont(132);
   TText *text = pt->AddText("Title");
   pt->Draw();
   Name->Modified();
   Name->cd();
   Name->SetSelected(Name);
}
