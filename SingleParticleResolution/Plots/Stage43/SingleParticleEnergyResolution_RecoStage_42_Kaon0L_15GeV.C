{
//=========Macro generated from canvas: Name/Title
//=========  (Tue Oct 20 18:44:11 2015) by ROOT version5.34/30
   TCanvas *Name = new TCanvas("Name", "Title",0,0,600,500);
   gStyle->SetOptStat(0);
   Name->SetHighLightColor(2);
   Name->Range(-3.75,-101.1938,33.75,910.7438);
   Name->SetFillColor(0);
   Name->SetBorderMode(0);
   Name->SetBorderSize(2);
   Name->SetTickx(1);
   Name->SetTicky(1);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   
   TH1F *Name = new TH1F("Name","Title",50,0,30);
   Name->SetBinContent(5,1);
   Name->SetBinContent(6,1);
   Name->SetBinContent(7,2);
   Name->SetBinContent(8,1);
   Name->SetBinContent(9,2);
   Name->SetBinContent(10,3);
   Name->SetBinContent(11,2);
   Name->SetBinContent(12,12);
   Name->SetBinContent(13,12);
   Name->SetBinContent(14,8);
   Name->SetBinContent(15,22);
   Name->SetBinContent(16,42);
   Name->SetBinContent(17,70);
   Name->SetBinContent(18,127);
   Name->SetBinContent(19,224);
   Name->SetBinContent(20,319);
   Name->SetBinContent(21,498);
   Name->SetBinContent(22,687);
   Name->SetBinContent(23,771);
   Name->SetBinContent(24,762);
   Name->SetBinContent(25,685);
   Name->SetBinContent(26,628);
   Name->SetBinContent(27,601);
   Name->SetBinContent(28,480);
   Name->SetBinContent(29,333);
   Name->SetBinContent(30,274);
   Name->SetBinContent(31,153);
   Name->SetBinContent(32,104);
   Name->SetBinContent(33,83);
   Name->SetBinContent(34,51);
   Name->SetBinContent(35,30);
   Name->SetBinContent(36,10);
   Name->SetBinContent(37,16);
   Name->SetBinContent(38,5);
   Name->SetBinContent(39,5);
   Name->SetBinContent(40,2);
   Name->SetBinContent(43,1);
   Name->SetBinContent(44,1);
   Name->SetBinContent(51,1);
   Name->SetEntries(7029);
   Name->SetStats(0);
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,30);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(134.8246);
   Gaussian->SetNDF(35);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,742.4271);
   Gaussian->SetParError(0,11.46273);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,14.48945);
   Gaussian->SetParError(1,0.02755637);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,2.222436);
   Gaussian->SetParError(2,0.02146691);
   Gaussian->SetParLimits(2,0,23.36595);
   Name->GetListOfFunctions()->Add(Gaussian);
   Name->SetLineWidth(2);
   Name->SetMarkerStyle(20);
   Name->GetXaxis()->SetTitle("PFO Energy [GeV]");
   Name->GetXaxis()->SetNdivisions(505);
   Name->GetXaxis()->SetLabelFont(132);
   Name->GetXaxis()->SetLabelSize(0.03);
   Name->GetXaxis()->SetTitleSize(0.036);
   Name->GetXaxis()->SetTitleFont(132);
   Name->GetYaxis()->SetTitle("Entries");
   Name->GetYaxis()->SetLabelFont(132);
   Name->GetYaxis()->SetLabelSize(0.03);
   Name->GetYaxis()->SetTitleSize(0.036);
   Name->GetYaxis()->SetTitleFont(132);
   Name->GetZaxis()->SetLabelFont(132);
   Name->GetZaxis()->SetLabelSize(0.03);
   Name->GetZaxis()->SetTitleSize(0.036);
   Name->GetZaxis()->SetTitleFont(132);
   Name->Draw("");
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,50);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(134.8246);
   Gaussian->SetNDF(35);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,742.4271);
   Gaussian->SetParError(0,11.46273);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,14.48945);
   Gaussian->SetParError(1,0.02755637);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,2.222436);
   Gaussian->SetParError(2,0.02146691);
   Gaussian->SetParLimits(2,0,23.36595);
   Gaussian->Draw("same");
   
   TPaveText *pt = new TPaveText(0.01,0.945,0.1004698,0.995,"blNDC");
   pt->SetName("title");
   pt->SetBorderSize(1);
   pt->SetFillColor(0);
   pt->SetLineWidth(2);
   pt->SetTextFont(132);
   TText *text = pt->AddText("Title");
   pt->Draw();
   Name->Modified();
   Name->cd();
   Name->SetSelected(Name);
}
