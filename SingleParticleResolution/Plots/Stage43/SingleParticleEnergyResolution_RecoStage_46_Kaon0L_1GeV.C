{
//=========Macro generated from canvas: Name/Title
//=========  (Tue Oct 20 18:44:37 2015) by ROOT version5.34/30
   TCanvas *Name = new TCanvas("Name", "Title",0,0,600,500);
   gStyle->SetOptStat(0);
   Name->SetHighLightColor(2);
   Name->Range(-0.25,-32.55,2.25,292.95);
   Name->SetFillColor(0);
   Name->SetBorderMode(0);
   Name->SetBorderSize(2);
   Name->SetTickx(1);
   Name->SetTicky(1);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   
   TH1F *Name = new TH1F("Name","Title",50,0,2);
   Name->SetBinContent(7,77);
   Name->SetBinContent(8,88);
   Name->SetBinContent(9,106);
   Name->SetBinContent(10,146);
   Name->SetBinContent(11,139);
   Name->SetBinContent(12,166);
   Name->SetBinContent(13,148);
   Name->SetBinContent(14,203);
   Name->SetBinContent(15,211);
   Name->SetBinContent(16,210);
   Name->SetBinContent(17,203);
   Name->SetBinContent(18,196);
   Name->SetBinContent(19,211);
   Name->SetBinContent(20,248);
   Name->SetBinContent(21,214);
   Name->SetBinContent(22,201);
   Name->SetBinContent(23,188);
   Name->SetBinContent(24,191);
   Name->SetBinContent(25,156);
   Name->SetBinContent(26,170);
   Name->SetBinContent(27,153);
   Name->SetBinContent(28,169);
   Name->SetBinContent(29,133);
   Name->SetBinContent(30,142);
   Name->SetBinContent(31,106);
   Name->SetBinContent(32,90);
   Name->SetBinContent(33,88);
   Name->SetBinContent(34,85);
   Name->SetBinContent(35,65);
   Name->SetBinContent(36,59);
   Name->SetBinContent(37,78);
   Name->SetBinContent(38,55);
   Name->SetBinContent(39,57);
   Name->SetBinContent(40,42);
   Name->SetBinContent(41,37);
   Name->SetBinContent(42,33);
   Name->SetBinContent(43,39);
   Name->SetBinContent(44,35);
   Name->SetBinContent(45,32);
   Name->SetBinContent(46,27);
   Name->SetBinContent(47,29);
   Name->SetBinContent(48,9);
   Name->SetBinContent(49,16);
   Name->SetBinContent(50,15);
   Name->SetBinContent(51,150);
   Name->SetEntries(5216);
   Name->SetStats(0);
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,2);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(121.0089);
   Gaussian->SetNDF(41);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,195.7847);
   Gaussian->SetParError(0,3.762464);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,0.7842603);
   Gaussian->SetParError(1,0.009370913);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,0.4589378);
   Gaussian->SetParError(2,0.009498342);
   Gaussian->SetParLimits(2,0,3.790393);
   Name->GetListOfFunctions()->Add(Gaussian);
   Name->SetLineWidth(2);
   Name->SetMarkerStyle(20);
   Name->GetXaxis()->SetTitle("PFO Energy [GeV]");
   Name->GetXaxis()->SetNdivisions(505);
   Name->GetXaxis()->SetLabelFont(132);
   Name->GetXaxis()->SetLabelSize(0.03);
   Name->GetXaxis()->SetTitleSize(0.036);
   Name->GetXaxis()->SetTitleFont(132);
   Name->GetYaxis()->SetTitle("Entries");
   Name->GetYaxis()->SetLabelFont(132);
   Name->GetYaxis()->SetLabelSize(0.03);
   Name->GetYaxis()->SetTitleSize(0.036);
   Name->GetYaxis()->SetTitleFont(132);
   Name->GetZaxis()->SetLabelFont(132);
   Name->GetZaxis()->SetLabelSize(0.03);
   Name->GetZaxis()->SetTitleSize(0.036);
   Name->GetZaxis()->SetTitleFont(132);
   Name->Draw("");
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,50);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(121.0089);
   Gaussian->SetNDF(41);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,195.7847);
   Gaussian->SetParError(0,3.762464);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,0.7842603);
   Gaussian->SetParError(1,0.009370913);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,0.4589378);
   Gaussian->SetParError(2,0.009498342);
   Gaussian->SetParLimits(2,0,3.790393);
   Gaussian->Draw("same");
   
   TPaveText *pt = new TPaveText(0.01,0.945,0.1004698,0.995,"blNDC");
   pt->SetName("title");
   pt->SetBorderSize(1);
   pt->SetFillColor(0);
   pt->SetLineWidth(2);
   pt->SetTextFont(132);
   TText *text = pt->AddText("Title");
   pt->Draw();
   Name->Modified();
   Name->cd();
   Name->SetSelected(Name);
}
