{
//=========Macro generated from canvas: Name/Title
//=========  (Tue Oct 20 18:44:34 2015) by ROOT version5.34/30
   TCanvas *Name = new TCanvas("Name", "Title",0,0,600,500);
   gStyle->SetOptStat(0);
   Name->SetHighLightColor(2);
   Name->Range(-6.25,-119.175,56.25,1072.575);
   Name->SetFillColor(0);
   Name->SetBorderMode(0);
   Name->SetBorderSize(2);
   Name->SetTickx(1);
   Name->SetTicky(1);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   
   TH1F *Name = new TH1F("Name","Title",50,0,50);
   Name->SetBinContent(5,1);
   Name->SetBinContent(7,2);
   Name->SetBinContent(9,4);
   Name->SetBinContent(10,3);
   Name->SetBinContent(11,3);
   Name->SetBinContent(12,8);
   Name->SetBinContent(13,10);
   Name->SetBinContent(14,9);
   Name->SetBinContent(15,14);
   Name->SetBinContent(16,15);
   Name->SetBinContent(17,28);
   Name->SetBinContent(18,39);
   Name->SetBinContent(19,84);
   Name->SetBinContent(20,162);
   Name->SetBinContent(21,278);
   Name->SetBinContent(22,454);
   Name->SetBinContent(23,609);
   Name->SetBinContent(24,754);
   Name->SetBinContent(25,908);
   Name->SetBinContent(26,869);
   Name->SetBinContent(27,818);
   Name->SetBinContent(28,726);
   Name->SetBinContent(29,481);
   Name->SetBinContent(30,344);
   Name->SetBinContent(31,193);
   Name->SetBinContent(32,117);
   Name->SetBinContent(33,53);
   Name->SetBinContent(34,38);
   Name->SetBinContent(35,13);
   Name->SetBinContent(36,3);
   Name->SetBinContent(37,4);
   Name->SetBinContent(38,2);
   Name->SetBinContent(39,2);
   Name->SetBinContent(40,1);
   Name->SetEntries(7049);
   Name->SetStats(0);
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,50);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(75.70385);
   Gaussian->SetNDF(31);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,888.5814);
   Gaussian->SetParError(0,13.43634);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,25.18939);
   Gaussian->SetParError(1,0.03750339);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,3.130772);
   Gaussian->SetParError(2,0.02890233);
   Gaussian->SetParLimits(2,0,33.48563);
   Name->GetListOfFunctions()->Add(Gaussian);
   Name->SetLineWidth(2);
   Name->SetMarkerStyle(20);
   Name->GetXaxis()->SetTitle("PFO Energy [GeV]");
   Name->GetXaxis()->SetNdivisions(505);
   Name->GetXaxis()->SetLabelFont(132);
   Name->GetXaxis()->SetLabelSize(0.03);
   Name->GetXaxis()->SetTitleSize(0.036);
   Name->GetXaxis()->SetTitleFont(132);
   Name->GetYaxis()->SetTitle("Entries");
   Name->GetYaxis()->SetLabelFont(132);
   Name->GetYaxis()->SetLabelSize(0.03);
   Name->GetYaxis()->SetTitleSize(0.036);
   Name->GetYaxis()->SetTitleFont(132);
   Name->GetZaxis()->SetLabelFont(132);
   Name->GetZaxis()->SetLabelSize(0.03);
   Name->GetZaxis()->SetTitleSize(0.036);
   Name->GetZaxis()->SetTitleFont(132);
   Name->Draw("");
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,50);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(75.70385);
   Gaussian->SetNDF(31);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,888.5814);
   Gaussian->SetParError(0,13.43634);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,25.18939);
   Gaussian->SetParError(1,0.03750339);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,3.130772);
   Gaussian->SetParError(2,0.02890233);
   Gaussian->SetParLimits(2,0,33.48563);
   Gaussian->Draw("same");
   
   TPaveText *pt = new TPaveText(0.01,0.945,0.1004698,0.995,"blNDC");
   pt->SetName("title");
   pt->SetBorderSize(1);
   pt->SetFillColor(0);
   pt->SetLineWidth(2);
   pt->SetTextFont(132);
   TText *text = pt->AddText("Title");
   pt->Draw();
   Name->Modified();
   Name->cd();
   Name->SetSelected(Name);
}
