{
//=========Macro generated from canvas: Name/Title
//=========  (Fri Oct 23 12:31:04 2015) by ROOT version5.34/30
   TCanvas *Name = new TCanvas("Name", "Title",0,0,600,500);
   gStyle->SetOptStat(0);
   Name->SetHighLightColor(2);
   Name->Range(-2.25,-81.63751,20.25,734.7375);
   Name->SetFillColor(0);
   Name->SetBorderMode(0);
   Name->SetBorderSize(2);
   Name->SetTickx(1);
   Name->SetTicky(1);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   
   TH1F *Name = new TH1F("Name","Title",50,0,18);
   Name->SetBinContent(6,1);
   Name->SetBinContent(7,4);
   Name->SetBinContent(8,3);
   Name->SetBinContent(9,5);
   Name->SetBinContent(10,5);
   Name->SetBinContent(11,4);
   Name->SetBinContent(12,13);
   Name->SetBinContent(13,11);
   Name->SetBinContent(14,28);
   Name->SetBinContent(15,37);
   Name->SetBinContent(16,85);
   Name->SetBinContent(17,117);
   Name->SetBinContent(18,190);
   Name->SetBinContent(19,271);
   Name->SetBinContent(20,338);
   Name->SetBinContent(21,400);
   Name->SetBinContent(22,473);
   Name->SetBinContent(23,574);
   Name->SetBinContent(24,609);
   Name->SetBinContent(25,622);
   Name->SetBinContent(26,544);
   Name->SetBinContent(27,506);
   Name->SetBinContent(28,456);
   Name->SetBinContent(29,386);
   Name->SetBinContent(30,298);
   Name->SetBinContent(31,232);
   Name->SetBinContent(32,170);
   Name->SetBinContent(33,133);
   Name->SetBinContent(34,78);
   Name->SetBinContent(35,43);
   Name->SetBinContent(36,32);
   Name->SetBinContent(37,28);
   Name->SetBinContent(38,25);
   Name->SetBinContent(39,11);
   Name->SetBinContent(40,3);
   Name->SetBinContent(41,6);
   Name->SetBinContent(42,2);
   Name->SetBinContent(44,1);
   Name->SetBinContent(45,1);
   Name->SetBinContent(46,1);
   Name->SetBinContent(47,2);
   Name->SetBinContent(49,2);
   Name->SetBinContent(51,1);
   Name->SetEntries(6751);
   Name->SetStats(0);
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,18);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(69.8746);
   Gaussian->SetNDF(39);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,591.0914);
   Gaussian->SetParError(0,9.075507);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,8.798613);
   Gaussian->SetParError(1,0.02008705);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,1.62315);
   Gaussian->SetParError(2,0.01506306);
   Gaussian->SetParLimits(2,0,16.88729);
   Name->GetListOfFunctions()->Add(Gaussian);
   Name->SetLineWidth(2);
   Name->SetMarkerStyle(20);
   Name->GetXaxis()->SetTitle("PFO Energy [GeV]");
   Name->GetXaxis()->SetNdivisions(505);
   Name->GetXaxis()->SetLabelFont(132);
   Name->GetXaxis()->SetLabelSize(0.03);
   Name->GetXaxis()->SetTitleSize(0.036);
   Name->GetXaxis()->SetTitleFont(132);
   Name->GetYaxis()->SetTitle("Entries");
   Name->GetYaxis()->SetLabelFont(132);
   Name->GetYaxis()->SetLabelSize(0.03);
   Name->GetYaxis()->SetTitleSize(0.036);
   Name->GetYaxis()->SetTitleFont(132);
   Name->GetZaxis()->SetLabelFont(132);
   Name->GetZaxis()->SetLabelSize(0.03);
   Name->GetZaxis()->SetTitleSize(0.036);
   Name->GetZaxis()->SetTitleFont(132);
   Name->Draw("");
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,50);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(69.8746);
   Gaussian->SetNDF(39);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,591.0914);
   Gaussian->SetParError(0,9.075507);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,8.798613);
   Gaussian->SetParError(1,0.02008705);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,1.62315);
   Gaussian->SetParError(2,0.01506306);
   Gaussian->SetParLimits(2,0,16.88729);
   Gaussian->Draw("same");
   
   TPaveText *pt = new TPaveText(0.01,0.945,0.1004698,0.995,"blNDC");
   pt->SetName("title");
   pt->SetBorderSize(1);
   pt->SetFillColor(0);
   pt->SetLineWidth(2);
   pt->SetTextFont(132);
   TText *text = pt->AddText("Title");
   pt->Draw();
   Name->Modified();
   Name->cd();
   Name->SetSelected(Name);
}
