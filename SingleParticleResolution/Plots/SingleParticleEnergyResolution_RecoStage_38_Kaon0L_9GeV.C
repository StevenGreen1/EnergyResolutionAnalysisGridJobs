{
//=========Macro generated from canvas: Name/Title
//=========  (Fri Oct 23 09:12:58 2015) by ROOT version5.34/30
   TCanvas *Name = new TCanvas("Name", "Title",0,0,600,500);
   gStyle->SetOptStat(0);
   Name->SetHighLightColor(2);
   Name->Range(-2.25,-80.58751,20.25,725.2875);
   Name->SetFillColor(0);
   Name->SetBorderMode(0);
   Name->SetBorderSize(2);
   Name->SetTickx(1);
   Name->SetTicky(1);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   
   TH1F *Name = new TH1F("Name","Title",50,0,18);
   Name->SetBinContent(6,3);
   Name->SetBinContent(7,4);
   Name->SetBinContent(8,4);
   Name->SetBinContent(9,6);
   Name->SetBinContent(10,4);
   Name->SetBinContent(11,5);
   Name->SetBinContent(12,10);
   Name->SetBinContent(13,17);
   Name->SetBinContent(14,36);
   Name->SetBinContent(15,51);
   Name->SetBinContent(16,107);
   Name->SetBinContent(17,177);
   Name->SetBinContent(18,223);
   Name->SetBinContent(19,319);
   Name->SetBinContent(20,413);
   Name->SetBinContent(21,470);
   Name->SetBinContent(22,562);
   Name->SetBinContent(23,606);
   Name->SetBinContent(24,614);
   Name->SetBinContent(25,565);
   Name->SetBinContent(26,542);
   Name->SetBinContent(27,437);
   Name->SetBinContent(28,383);
   Name->SetBinContent(29,306);
   Name->SetBinContent(30,240);
   Name->SetBinContent(31,191);
   Name->SetBinContent(32,147);
   Name->SetBinContent(33,99);
   Name->SetBinContent(34,70);
   Name->SetBinContent(35,51);
   Name->SetBinContent(36,30);
   Name->SetBinContent(37,16);
   Name->SetBinContent(38,24);
   Name->SetBinContent(39,9);
   Name->SetBinContent(40,8);
   Name->SetBinContent(41,4);
   Name->SetBinContent(42,4);
   Name->SetBinContent(43,2);
   Name->SetBinContent(45,1);
   Name->SetBinContent(46,2);
   Name->SetBinContent(47,1);
   Name->SetBinContent(49,1);
   Name->SetBinContent(50,1);
   Name->SetBinContent(51,1);
   Name->SetEntries(6766);
   Name->SetStats(0);
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,18);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(111.2703);
   Gaussian->SetNDF(40);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,588.6751);
   Gaussian->SetParError(0,9.16133);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,8.558991);
   Gaussian->SetParError(1,0.02077316);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,1.623347);
   Gaussian->SetParError(2,0.01556533);
   Gaussian->SetParLimits(2,0,17.12431);
   Name->GetListOfFunctions()->Add(Gaussian);
   Name->SetLineWidth(2);
   Name->SetMarkerStyle(20);
   Name->GetXaxis()->SetTitle("PFO Energy [GeV]");
   Name->GetXaxis()->SetNdivisions(505);
   Name->GetXaxis()->SetLabelFont(132);
   Name->GetXaxis()->SetLabelSize(0.03);
   Name->GetXaxis()->SetTitleSize(0.036);
   Name->GetXaxis()->SetTitleFont(132);
   Name->GetYaxis()->SetTitle("Entries");
   Name->GetYaxis()->SetLabelFont(132);
   Name->GetYaxis()->SetLabelSize(0.03);
   Name->GetYaxis()->SetTitleSize(0.036);
   Name->GetYaxis()->SetTitleFont(132);
   Name->GetZaxis()->SetLabelFont(132);
   Name->GetZaxis()->SetLabelSize(0.03);
   Name->GetZaxis()->SetTitleSize(0.036);
   Name->GetZaxis()->SetTitleFont(132);
   Name->Draw("");
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,50);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(111.2703);
   Gaussian->SetNDF(40);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,588.6751);
   Gaussian->SetParError(0,9.16133);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,8.558991);
   Gaussian->SetParError(1,0.02077316);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,1.623347);
   Gaussian->SetParError(2,0.01556533);
   Gaussian->SetParLimits(2,0,17.12431);
   Gaussian->Draw("same");
   
   TPaveText *pt = new TPaveText(0.01,0.945,0.1004698,0.995,"blNDC");
   pt->SetName("title");
   pt->SetBorderSize(1);
   pt->SetFillColor(0);
   pt->SetLineWidth(2);
   pt->SetTextFont(132);
   TText *text = pt->AddText("Title");
   pt->Draw();
   Name->Modified();
   Name->cd();
   Name->SetSelected(Name);
}
