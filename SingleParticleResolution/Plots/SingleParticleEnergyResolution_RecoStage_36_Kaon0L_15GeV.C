{
//=========Macro generated from canvas: Name/Title
//=========  (Fri Oct 23 12:31:05 2015) by ROOT version5.34/30
   TCanvas *Name = new TCanvas("Name", "Title",0,0,600,500);
   gStyle->SetOptStat(0);
   Name->SetHighLightColor(2);
   Name->Range(-3.75,-108.15,33.75,973.35);
   Name->SetFillColor(0);
   Name->SetBorderMode(0);
   Name->SetBorderSize(2);
   Name->SetTickx(1);
   Name->SetTicky(1);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   
   TH1F *Name = new TH1F("Name","Title",50,0,30);
   Name->SetBinContent(5,1);
   Name->SetBinContent(7,1);
   Name->SetBinContent(8,2);
   Name->SetBinContent(9,2);
   Name->SetBinContent(10,2);
   Name->SetBinContent(11,5);
   Name->SetBinContent(12,2);
   Name->SetBinContent(13,5);
   Name->SetBinContent(14,8);
   Name->SetBinContent(15,16);
   Name->SetBinContent(16,20);
   Name->SetBinContent(17,40);
   Name->SetBinContent(18,65);
   Name->SetBinContent(19,142);
   Name->SetBinContent(20,223);
   Name->SetBinContent(21,352);
   Name->SetBinContent(22,565);
   Name->SetBinContent(23,695);
   Name->SetBinContent(24,798);
   Name->SetBinContent(25,824);
   Name->SetBinContent(26,808);
   Name->SetBinContent(27,684);
   Name->SetBinContent(28,557);
   Name->SetBinContent(29,437);
   Name->SetBinContent(30,305);
   Name->SetBinContent(31,190);
   Name->SetBinContent(32,115);
   Name->SetBinContent(33,64);
   Name->SetBinContent(34,40);
   Name->SetBinContent(35,20);
   Name->SetBinContent(36,13);
   Name->SetBinContent(37,5);
   Name->SetBinContent(38,4);
   Name->SetBinContent(39,1);
   Name->SetBinContent(42,1);
   Name->SetBinContent(44,1);
   Name->SetEntries(7013);
   Name->SetStats(0);
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,30);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(60.78634);
   Gaussian->SetNDF(33);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,816.4832);
   Gaussian->SetParError(0,12.49737);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,14.86209);
   Gaussian->SetParError(1,0.02459687);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,2.038178);
   Gaussian->SetParError(2,0.01938558);
   Gaussian->SetParLimits(2,0,21.20676);
   Name->GetListOfFunctions()->Add(Gaussian);
   Name->SetLineWidth(2);
   Name->SetMarkerStyle(20);
   Name->GetXaxis()->SetTitle("PFO Energy [GeV]");
   Name->GetXaxis()->SetNdivisions(505);
   Name->GetXaxis()->SetLabelFont(132);
   Name->GetXaxis()->SetLabelSize(0.03);
   Name->GetXaxis()->SetTitleSize(0.036);
   Name->GetXaxis()->SetTitleFont(132);
   Name->GetYaxis()->SetTitle("Entries");
   Name->GetYaxis()->SetLabelFont(132);
   Name->GetYaxis()->SetLabelSize(0.03);
   Name->GetYaxis()->SetTitleSize(0.036);
   Name->GetYaxis()->SetTitleFont(132);
   Name->GetZaxis()->SetLabelFont(132);
   Name->GetZaxis()->SetLabelSize(0.03);
   Name->GetZaxis()->SetTitleSize(0.036);
   Name->GetZaxis()->SetTitleFont(132);
   Name->Draw("");
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,50);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(60.78634);
   Gaussian->SetNDF(33);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,816.4832);
   Gaussian->SetParError(0,12.49737);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,14.86209);
   Gaussian->SetParError(1,0.02459687);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,2.038178);
   Gaussian->SetParError(2,0.01938558);
   Gaussian->SetParLimits(2,0,21.20676);
   Gaussian->Draw("same");
   
   TPaveText *pt = new TPaveText(0.01,0.945,0.1004698,0.995,"blNDC");
   pt->SetName("title");
   pt->SetBorderSize(1);
   pt->SetFillColor(0);
   pt->SetLineWidth(2);
   pt->SetTextFont(132);
   TText *text = pt->AddText("Title");
   pt->Draw();
   Name->Modified();
   Name->cd();
   Name->SetSelected(Name);
}
