{
//=========Macro generated from canvas: Name/Title
//=========  (Fri Oct 23 09:13:23 2015) by ROOT version5.34/30
   TCanvas *Name = new TCanvas("Name", "Title",0,0,600,500);
   gStyle->SetOptStat(0);
   Name->SetHighLightColor(2);
   Name->Range(-2.25,-82.16251,20.25,739.4625);
   Name->SetFillColor(0);
   Name->SetBorderMode(0);
   Name->SetBorderSize(2);
   Name->SetTickx(1);
   Name->SetTicky(1);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   
   TH1F *Name = new TH1F("Name","Title",50,0,18);
   Name->SetBinContent(6,4);
   Name->SetBinContent(7,5);
   Name->SetBinContent(8,3);
   Name->SetBinContent(9,6);
   Name->SetBinContent(10,3);
   Name->SetBinContent(11,5);
   Name->SetBinContent(12,12);
   Name->SetBinContent(13,19);
   Name->SetBinContent(14,43);
   Name->SetBinContent(15,62);
   Name->SetBinContent(16,126);
   Name->SetBinContent(17,187);
   Name->SetBinContent(18,260);
   Name->SetBinContent(19,355);
   Name->SetBinContent(20,449);
   Name->SetBinContent(21,471);
   Name->SetBinContent(22,579);
   Name->SetBinContent(23,626);
   Name->SetBinContent(24,566);
   Name->SetBinContent(25,569);
   Name->SetBinContent(26,510);
   Name->SetBinContent(27,406);
   Name->SetBinContent(28,352);
   Name->SetBinContent(29,271);
   Name->SetBinContent(30,244);
   Name->SetBinContent(31,177);
   Name->SetBinContent(32,119);
   Name->SetBinContent(33,101);
   Name->SetBinContent(34,71);
   Name->SetBinContent(35,46);
   Name->SetBinContent(36,31);
   Name->SetBinContent(37,23);
   Name->SetBinContent(38,17);
   Name->SetBinContent(39,18);
   Name->SetBinContent(40,7);
   Name->SetBinContent(41,10);
   Name->SetBinContent(42,6);
   Name->SetBinContent(43,2);
   Name->SetBinContent(44,2);
   Name->SetBinContent(45,1);
   Name->SetBinContent(46,1);
   Name->SetBinContent(47,2);
   Name->SetBinContent(49,1);
   Name->SetBinContent(51,2);
   Name->SetEntries(6770);
   Name->SetStats(0);
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,18);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(157.7301);
   Gaussian->SetNDF(40);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,581.5223);
   Gaussian->SetParError(0,9.110037);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,8.458935);
   Gaussian->SetParError(1,0.02142488);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,1.632582);
   Gaussian->SetParError(2,0.01584169);
   Gaussian->SetParLimits(2,0,17.52948);
   Name->GetListOfFunctions()->Add(Gaussian);
   Name->SetLineWidth(2);
   Name->SetMarkerStyle(20);
   Name->GetXaxis()->SetTitle("PFO Energy [GeV]");
   Name->GetXaxis()->SetNdivisions(505);
   Name->GetXaxis()->SetLabelFont(132);
   Name->GetXaxis()->SetLabelSize(0.03);
   Name->GetXaxis()->SetTitleSize(0.036);
   Name->GetXaxis()->SetTitleFont(132);
   Name->GetYaxis()->SetTitle("Entries");
   Name->GetYaxis()->SetLabelFont(132);
   Name->GetYaxis()->SetLabelSize(0.03);
   Name->GetYaxis()->SetTitleSize(0.036);
   Name->GetYaxis()->SetTitleFont(132);
   Name->GetZaxis()->SetLabelFont(132);
   Name->GetZaxis()->SetLabelSize(0.03);
   Name->GetZaxis()->SetTitleSize(0.036);
   Name->GetZaxis()->SetTitleFont(132);
   Name->Draw("");
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,50);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(157.7301);
   Gaussian->SetNDF(40);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,581.5223);
   Gaussian->SetParError(0,9.110037);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,8.458935);
   Gaussian->SetParError(1,0.02142488);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,1.632582);
   Gaussian->SetParError(2,0.01584169);
   Gaussian->SetParLimits(2,0,17.52948);
   Gaussian->Draw("same");
   
   TPaveText *pt = new TPaveText(0.01,0.945,0.1004698,0.995,"blNDC");
   pt->SetName("title");
   pt->SetBorderSize(1);
   pt->SetFillColor(0);
   pt->SetLineWidth(2);
   pt->SetTextFont(132);
   TText *text = pt->AddText("Title");
   pt->Draw();
   Name->Modified();
   Name->cd();
   Name->SetSelected(Name);
}
