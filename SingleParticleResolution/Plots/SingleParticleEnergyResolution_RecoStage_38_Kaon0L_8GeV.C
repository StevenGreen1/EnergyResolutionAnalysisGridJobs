{
//=========Macro generated from canvas: Name/Title
//=========  (Fri Oct 23 09:12:58 2015) by ROOT version5.34/30
   TCanvas *Name = new TCanvas("Name", "Title",0,0,600,500);
   gStyle->SetOptStat(0);
   Name->SetHighLightColor(2);
   Name->Range(-2,-80.19376,18,721.7438);
   Name->SetFillColor(0);
   Name->SetBorderMode(0);
   Name->SetBorderSize(2);
   Name->SetTickx(1);
   Name->SetTicky(1);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   
   TH1F *Name = new TH1F("Name","Title",50,0,16);
   Name->SetBinContent(6,1);
   Name->SetBinContent(7,3);
   Name->SetBinContent(8,3);
   Name->SetBinContent(9,1);
   Name->SetBinContent(10,8);
   Name->SetBinContent(11,9);
   Name->SetBinContent(12,16);
   Name->SetBinContent(13,27);
   Name->SetBinContent(14,42);
   Name->SetBinContent(15,71);
   Name->SetBinContent(16,126);
   Name->SetBinContent(17,202);
   Name->SetBinContent(18,260);
   Name->SetBinContent(19,402);
   Name->SetBinContent(20,421);
   Name->SetBinContent(21,491);
   Name->SetBinContent(22,585);
   Name->SetBinContent(23,611);
   Name->SetBinContent(24,598);
   Name->SetBinContent(25,518);
   Name->SetBinContent(26,452);
   Name->SetBinContent(27,463);
   Name->SetBinContent(28,368);
   Name->SetBinContent(29,265);
   Name->SetBinContent(30,236);
   Name->SetBinContent(31,172);
   Name->SetBinContent(32,151);
   Name->SetBinContent(33,88);
   Name->SetBinContent(34,74);
   Name->SetBinContent(35,59);
   Name->SetBinContent(36,40);
   Name->SetBinContent(37,30);
   Name->SetBinContent(38,18);
   Name->SetBinContent(39,21);
   Name->SetBinContent(40,9);
   Name->SetBinContent(41,9);
   Name->SetBinContent(42,3);
   Name->SetBinContent(43,4);
   Name->SetBinContent(44,2);
   Name->SetBinContent(45,1);
   Name->SetBinContent(50,1);
   Name->SetBinContent(51,4);
   Name->SetEntries(6865);
   Name->SetStats(0);
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,16);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(168.7053);
   Gaussian->SetNDF(38);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,572.5496);
   Gaussian->SetParError(0,8.946526);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,7.512814);
   Gaussian->SetParError(1,0.01955237);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,1.492238);
   Gaussian->SetParError(2,0.01452914);
   Gaussian->SetParLimits(2,0,15.82425);
   Name->GetListOfFunctions()->Add(Gaussian);
   Name->SetLineWidth(2);
   Name->SetMarkerStyle(20);
   Name->GetXaxis()->SetTitle("PFO Energy [GeV]");
   Name->GetXaxis()->SetNdivisions(505);
   Name->GetXaxis()->SetLabelFont(132);
   Name->GetXaxis()->SetLabelSize(0.03);
   Name->GetXaxis()->SetTitleSize(0.036);
   Name->GetXaxis()->SetTitleFont(132);
   Name->GetYaxis()->SetTitle("Entries");
   Name->GetYaxis()->SetLabelFont(132);
   Name->GetYaxis()->SetLabelSize(0.03);
   Name->GetYaxis()->SetTitleSize(0.036);
   Name->GetYaxis()->SetTitleFont(132);
   Name->GetZaxis()->SetLabelFont(132);
   Name->GetZaxis()->SetLabelSize(0.03);
   Name->GetZaxis()->SetTitleSize(0.036);
   Name->GetZaxis()->SetTitleFont(132);
   Name->Draw("");
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,50);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(168.7053);
   Gaussian->SetNDF(38);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,572.5496);
   Gaussian->SetParError(0,8.946526);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,7.512814);
   Gaussian->SetParError(1,0.01955237);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,1.492238);
   Gaussian->SetParError(2,0.01452914);
   Gaussian->SetParLimits(2,0,15.82425);
   Gaussian->Draw("same");
   
   TPaveText *pt = new TPaveText(0.01,0.945,0.1004698,0.995,"blNDC");
   pt->SetName("title");
   pt->SetBorderSize(1);
   pt->SetFillColor(0);
   pt->SetLineWidth(2);
   pt->SetTextFont(132);
   TText *text = pt->AddText("Title");
   pt->Draw();
   Name->Modified();
   Name->cd();
   Name->SetSelected(Name);
}
