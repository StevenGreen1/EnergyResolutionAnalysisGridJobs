{
//=========Macro generated from canvas: Name/Title
//=========  (Fri Oct 23 12:31:10 2015) by ROOT version5.34/30
   TCanvas *Name = new TCanvas("Name", "Title",0,0,600,500);
   gStyle->SetOptStat(0);
   Name->SetHighLightColor(2);
   Name->Range(-2.25,-79.40626,20.25,714.6563);
   Name->SetFillColor(0);
   Name->SetBorderMode(0);
   Name->SetBorderSize(2);
   Name->SetTickx(1);
   Name->SetTicky(1);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   
   TH1F *Name = new TH1F("Name","Title",50,0,18);
   Name->SetBinContent(6,3);
   Name->SetBinContent(7,4);
   Name->SetBinContent(8,3);
   Name->SetBinContent(9,6);
   Name->SetBinContent(10,4);
   Name->SetBinContent(11,3);
   Name->SetBinContent(12,14);
   Name->SetBinContent(13,13);
   Name->SetBinContent(14,27);
   Name->SetBinContent(15,54);
   Name->SetBinContent(16,96);
   Name->SetBinContent(17,150);
   Name->SetBinContent(18,234);
   Name->SetBinContent(19,275);
   Name->SetBinContent(20,389);
   Name->SetBinContent(21,463);
   Name->SetBinContent(22,531);
   Name->SetBinContent(23,605);
   Name->SetBinContent(24,590);
   Name->SetBinContent(25,591);
   Name->SetBinContent(26,550);
   Name->SetBinContent(27,465);
   Name->SetBinContent(28,404);
   Name->SetBinContent(29,342);
   Name->SetBinContent(30,270);
   Name->SetBinContent(31,182);
   Name->SetBinContent(32,155);
   Name->SetBinContent(33,111);
   Name->SetBinContent(34,65);
   Name->SetBinContent(35,54);
   Name->SetBinContent(36,34);
   Name->SetBinContent(37,24);
   Name->SetBinContent(38,22);
   Name->SetBinContent(39,7);
   Name->SetBinContent(40,4);
   Name->SetBinContent(41,7);
   Name->SetBinContent(42,3);
   Name->SetBinContent(43,1);
   Name->SetBinContent(46,3);
   Name->SetBinContent(47,1);
   Name->SetBinContent(49,1);
   Name->SetBinContent(50,1);
   Name->SetBinContent(51,1);
   Name->SetEntries(6762);
   Name->SetStats(0);
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,18);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(97.08407);
   Gaussian->SetNDF(39);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,591.2619);
   Gaussian->SetParError(0,9.124805);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,8.642202);
   Gaussian->SetParError(1,0.02053848);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,1.618717);
   Gaussian->SetParError(2,0.01519746);
   Gaussian->SetParLimits(2,0,17.02311);
   Name->GetListOfFunctions()->Add(Gaussian);
   Name->SetLineWidth(2);
   Name->SetMarkerStyle(20);
   Name->GetXaxis()->SetTitle("PFO Energy [GeV]");
   Name->GetXaxis()->SetNdivisions(505);
   Name->GetXaxis()->SetLabelFont(132);
   Name->GetXaxis()->SetLabelSize(0.03);
   Name->GetXaxis()->SetTitleSize(0.036);
   Name->GetXaxis()->SetTitleFont(132);
   Name->GetYaxis()->SetTitle("Entries");
   Name->GetYaxis()->SetLabelFont(132);
   Name->GetYaxis()->SetLabelSize(0.03);
   Name->GetYaxis()->SetTitleSize(0.036);
   Name->GetYaxis()->SetTitleFont(132);
   Name->GetZaxis()->SetLabelFont(132);
   Name->GetZaxis()->SetLabelSize(0.03);
   Name->GetZaxis()->SetTitleSize(0.036);
   Name->GetZaxis()->SetTitleFont(132);
   Name->Draw("");
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,50);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(97.08407);
   Gaussian->SetNDF(39);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,591.2619);
   Gaussian->SetParError(0,9.124805);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,8.642202);
   Gaussian->SetParError(1,0.02053848);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,1.618717);
   Gaussian->SetParError(2,0.01519746);
   Gaussian->SetParLimits(2,0,17.02311);
   Gaussian->Draw("same");
   
   TPaveText *pt = new TPaveText(0.01,0.945,0.1004698,0.995,"blNDC");
   pt->SetName("title");
   pt->SetBorderSize(1);
   pt->SetFillColor(0);
   pt->SetLineWidth(2);
   pt->SetTextFont(132);
   TText *text = pt->AddText("Title");
   pt->Draw();
   Name->Modified();
   Name->cd();
   Name->SetSelected(Name);
}
