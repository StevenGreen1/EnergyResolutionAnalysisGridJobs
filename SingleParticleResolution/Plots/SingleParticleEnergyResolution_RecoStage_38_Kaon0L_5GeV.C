{
//=========Macro generated from canvas: Name/Title
//=========  (Fri Oct 23 09:12:56 2015) by ROOT version5.34/30
   TCanvas *Name = new TCanvas("Name", "Title",0,0,600,500);
   gStyle->SetOptStat(0);
   Name->SetHighLightColor(2);
   Name->Range(-1.25,-72.31876,11.25,650.8688);
   Name->SetFillColor(0);
   Name->SetBorderMode(0);
   Name->SetBorderSize(2);
   Name->SetTickx(1);
   Name->SetTicky(1);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   Name->SetFrameLineWidth(2);
   Name->SetFrameBorderMode(0);
   
   TH1F *Name = new TH1F("Name","Title",50,0,10);
   Name->SetBinContent(2,1);
   Name->SetBinContent(3,4);
   Name->SetBinContent(4,6);
   Name->SetBinContent(5,5);
   Name->SetBinContent(6,7);
   Name->SetBinContent(7,5);
   Name->SetBinContent(8,9);
   Name->SetBinContent(9,16);
   Name->SetBinContent(10,22);
   Name->SetBinContent(11,28);
   Name->SetBinContent(12,58);
   Name->SetBinContent(13,81);
   Name->SetBinContent(14,133);
   Name->SetBinContent(15,167);
   Name->SetBinContent(16,242);
   Name->SetBinContent(17,309);
   Name->SetBinContent(18,358);
   Name->SetBinContent(19,434);
   Name->SetBinContent(20,498);
   Name->SetBinContent(21,520);
   Name->SetBinContent(22,551);
   Name->SetBinContent(23,502);
   Name->SetBinContent(24,474);
   Name->SetBinContent(25,442);
   Name->SetBinContent(26,400);
   Name->SetBinContent(27,363);
   Name->SetBinContent(28,294);
   Name->SetBinContent(29,264);
   Name->SetBinContent(30,218);
   Name->SetBinContent(31,165);
   Name->SetBinContent(32,131);
   Name->SetBinContent(33,106);
   Name->SetBinContent(34,101);
   Name->SetBinContent(35,58);
   Name->SetBinContent(36,53);
   Name->SetBinContent(37,41);
   Name->SetBinContent(38,23);
   Name->SetBinContent(39,19);
   Name->SetBinContent(40,21);
   Name->SetBinContent(41,11);
   Name->SetBinContent(42,12);
   Name->SetBinContent(43,8);
   Name->SetBinContent(44,6);
   Name->SetBinContent(45,7);
   Name->SetBinContent(46,1);
   Name->SetBinContent(47,4);
   Name->SetBinContent(48,5);
   Name->SetBinContent(50,4);
   Name->SetBinContent(51,4);
   Name->SetEntries(7191);
   Name->SetStats(0);
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,10);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(183.7714);
   Gaussian->SetNDF(45);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,504.4174);
   Gaussian->SetParError(0,7.823282);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,4.516337);
   Gaussian->SetParError(1,0.01412125);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,1.107821);
   Gaussian->SetParError(2,0.01095825);
   Gaussian->SetParLimits(2,0,11.9081);
   Name->GetListOfFunctions()->Add(Gaussian);
   Name->SetLineWidth(2);
   Name->SetMarkerStyle(20);
   Name->GetXaxis()->SetTitle("PFO Energy [GeV]");
   Name->GetXaxis()->SetNdivisions(505);
   Name->GetXaxis()->SetLabelFont(132);
   Name->GetXaxis()->SetLabelSize(0.03);
   Name->GetXaxis()->SetTitleSize(0.036);
   Name->GetXaxis()->SetTitleFont(132);
   Name->GetYaxis()->SetTitle("Entries");
   Name->GetYaxis()->SetLabelFont(132);
   Name->GetYaxis()->SetLabelSize(0.03);
   Name->GetYaxis()->SetTitleSize(0.036);
   Name->GetYaxis()->SetTitleFont(132);
   Name->GetZaxis()->SetLabelFont(132);
   Name->GetZaxis()->SetLabelSize(0.03);
   Name->GetZaxis()->SetTitleSize(0.036);
   Name->GetZaxis()->SetTitleFont(132);
   Name->Draw("");
   
   TF1 *Gaussian = new TF1("Gaussian","gaus",0,50);
   Gaussian->SetFillColor(1);
   Gaussian->SetFillStyle(0);
   Gaussian->SetMarkerStyle(20);
   Gaussian->SetLineWidth(2);
   Gaussian->SetChisquare(183.7714);
   Gaussian->SetNDF(45);
   Gaussian->GetXaxis()->SetNdivisions(505);
   Gaussian->GetXaxis()->SetLabelFont(132);
   Gaussian->GetXaxis()->SetLabelSize(0.03);
   Gaussian->GetXaxis()->SetTitleSize(0.036);
   Gaussian->GetXaxis()->SetTitleFont(132);
   Gaussian->GetYaxis()->SetLabelFont(132);
   Gaussian->GetYaxis()->SetLabelSize(0.03);
   Gaussian->GetYaxis()->SetTitleSize(0.036);
   Gaussian->GetYaxis()->SetTitleFont(132);
   Gaussian->SetParameter(0,504.4174);
   Gaussian->SetParError(0,7.823282);
   Gaussian->SetParLimits(0,0,0);
   Gaussian->SetParameter(1,4.516337);
   Gaussian->SetParError(1,0.01412125);
   Gaussian->SetParLimits(1,0,0);
   Gaussian->SetParameter(2,1.107821);
   Gaussian->SetParError(2,0.01095825);
   Gaussian->SetParLimits(2,0,11.9081);
   Gaussian->Draw("same");
   
   TPaveText *pt = new TPaveText(0.01,0.945,0.1004698,0.995,"blNDC");
   pt->SetName("title");
   pt->SetBorderSize(1);
   pt->SetFillColor(0);
   pt->SetLineWidth(2);
   pt->SetTextFont(132);
   TText *text = pt->AddText("Title");
   pt->Draw();
   Name->Modified();
   Name->cd();
   Name->SetSelected(Name);
}
